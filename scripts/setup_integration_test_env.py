...
class IntegrationTestSetup:
    ...
    def check_configuration(self):
        ...
        config_files = [Path("config.yaml"), Path(".env"), Path("user_config.yaml")]
    ...
    def provide_usage_guidance(self):
        ...
        if self.checks_passed == self.checks_total:
            ...
            logger.info("Example:")
            logger.info(
                "  python scripts/test_ibkr_market_data_integration.py --symbols AAPL --duration 30 --verbose"
            )
        else:
            logger.warning(
                f"⚠️  {self.checks_total - self.checks_passed} checks failed!"
            )
