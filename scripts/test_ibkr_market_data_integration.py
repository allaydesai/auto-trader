...
from auto_trader.integrations.ibkr_client.historical_data_fetcher import (
...
class IBKRMarketDataIntegrationTests:
    ...
    async def setup(self) -> bool:
        ...
        try:
            ...
            self.market_data_manager = MarketDataManager(
                self.ibkr_client._ib, self.cache
            )

            # Add bar callback to distributor
            self.market_data_manager.add_subscriber(
                "integration_test", self.bar_callback
            )

            self.historical_fetcher = HistoricalDataFetcher(self.ibkr_client._ib)
    ...
    async def test_real_time_subscriptions(self):
        ...
        try:
            logger.info(
                f"Testing real-time subscriptions for {len(self.symbols)} symbols..."
            )
            ...
            assert (
                len(active_subs) == len(self.symbols)
    ...
    async def test_dynamic_subscription_management(self):
        ...
        try:
            ...
            for symbol in new_symbols:
                assert (
                    symbol not in active_subs
                ), f"{symbol} still in active subscriptions"
...
async def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="IBKR Market Data Integration Tests")
    parser.add_argument(
        "--symbols",
        default="AAPL,MSFT",
