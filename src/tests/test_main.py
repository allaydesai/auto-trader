...
class TestAutoTraderApp:
    ...
    def test_app_initialization(self, mock_settings) -> None:
    ...
    def test_app_initialization_default_settings(self) -> None:
    ...
    @pytest.mark.asyncio
    async def test_initialize_success(self, mock_settings) -> None:
        """Test successful application initialization."""
        app = AutoTraderApp(mock_settings)

        with patch("main.LoggerConfig") as mock_log_config_class, patch.object(
            app.config_loader, "validate_configuration"
        ) as mock_validate, patch.object(
            app.config_loader, "load_system_config"
        ) as mock_system_config, patch.object(
            app.config_loader, "load_user_preferences"
        ) as mock_user_prefs:
    ...
    @pytest.mark.asyncio
    async def test_initialize_validation_failure(self, mock_settings) -> None:
        """Test initialization with configuration validation failure."""
        app = AutoTraderApp(mock_settings)

        with patch("main.LoggerConfig"), patch.object(
            app.config_loader, "validate_configuration"
        ) as mock_validate:
    ...
    @pytest.mark.asyncio
    async def test_initialize_exception_handling(self, mock_settings) -> None:
    ...
    @pytest.mark.asyncio
    async def test_start_success(self, mock_settings) -> None:
        """Test successful application start."""
        app = AutoTraderApp(mock_settings)

        with patch.object(app, "initialize") as mock_init, patch.object(
            app, "_setup_signal_handlers"
        ) as mock_signal, patch.object(app, "_run_main_loop") as mock_main_loop:
    ...
    @pytest.mark.asyncio
    async def test_start_initialization_failure(self, mock_settings) -> None:
    ...
    @pytest.mark.asyncio
    async def test_shutdown(self, mock_settings) -> None:
    ...
    @pytest.mark.asyncio
    async def test_shutdown_not_running(self, mock_settings) -> None:
    ...
    @pytest.mark.asyncio
    async def test_run_main_loop_shutdown_signal(self, mock_settings) -> None:
        ...
        await asyncio.gather(app._run_main_loop(), trigger_shutdown())

        # Should exit cleanly

    @pytest.mark.asyncio
    async def test_run_main_loop_exception(self, mock_settings) -> None:
    ...
    def test_setup_signal_handlers(self, mock_settings) -> None:
...
class TestMainFunction:
    ...
    async def test_main_success(self) -> None:
        ...
        with patch("main.Settings") as mock_settings_class, patch(
            "main.AutoTraderApp"
        ) as mock_app_class:
            mock_settings = MagicMock()
            mock_settings_class.return_value = mock_settings
...
class TestMainScriptExecution:
    ...
    def test_main_script_success(self) -> None:
        ...
        with patch("main.asyncio.run") as mock_run, patch("main.sys.exit") as mock_exit:
            # Create a proper mock that doesn't return a coroutine
            mock_run.return_value = 0

            # Import and execute main block
            with patch("main.__name__", "__main__"):
                # Create a proper mock main function that doesn't leak coroutines
...
if __name__ == "__main__":
    ...
    def test_main_script_exception(self) -> None:
        """Test script execution with exception."""
        with patch("main.asyncio.run") as mock_run, patch(
            "main.sys.exit"
