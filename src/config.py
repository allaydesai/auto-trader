...
class IBKRConfig(BaseModel):
    ...
    port: int = Field(default=7497, ge=1024, le=65535, description="IBKR TWS/Gateway port")
...
class TradingConfig(BaseModel):
    ...
    default_timeframe: str = Field(default="15min", description="Default execution timeframe")
    order_timeout: int = Field(default=60, ge=10, le=300, description="Order timeout in seconds")
...
class UserPreferences(BaseModel):
    ...
    default_account_value: Decimal = Field(
        default=Decimal("10000"), ge=1000, description="Default account value for calculations"
    )
...
class Settings(BaseSettings):
    ...
    # File Paths
    config_file: Path = Field(default=Path("config.yaml"), description="System config file path")
...
class ConfigLoader:
    ...
    def validate_configuration(self) -> list[str]:
        ...
        try:
            ...
            if system_config.risk.min_account_balance > user_preferences.default_account_value:
                issues.append(
