...
async def complete_order_system(
    mock_ibkr_client, mock_risk_validator, temp_state_dir, mock_discord_notifier
):
...
class TestEndToEndIntegration:
    ...
    async def test_complete_market_order_workflow(
        self, complete_order_system, mock_discord_notifier, sample_order_requests
    ):
    ...
    async def test_order_modification_workflow(
        self, complete_order_system, sample_order_requests
    ):
    ...
    async def test_risk_rejection_workflow(
        ...
        assert (
            "Insufficient buying power; Position size exceeds limit"
            in call_args.kwargs["reason"]
        )
    ...
    async def test_state_persistence_across_restarts(
        ...
        await manager2.get_active_orders()
        all_order_statuses = []
        ...
        new_result = await manager2.place_market_order(
            sample_order_requests["market_buy"]
        )
        ...
        if all_order_statuses:
            for order_status in all_order_statuses:
                assert hasattr(
                    order_status, "symbol"
                ), "Recovered orders should have symbol attribute"
                assert hasattr(
                    order_status, "order_id"
                ), "Recovered orders should have order_id attribute"
