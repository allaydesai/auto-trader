...
**Plan ID:** {order.trade_plan_id}
    ...
    async def send_order_filled(
        self, order: Order, entry_price: Optional[Decimal] = None
    ) -> None:
        """
...
**Order ID:** {order.order_id}
    ...
    async def send_order_rejected(
        self, order_request: OrderRequest, reason: str
    ) -> None:
        """
...
**Order ID:** {entry_order.order_id}
    ...
    async def send_system_alert(self, alert_type: str, message: str) -> None:
        ...
        emoji_map = {"ERROR": "🚨", "WARNING": "🟡", "INFO": "ℹ️"}
        ...
        try:
            ...
            if e.response.status_code == 429:
                ...
                try:
                    ...
                    logger.error(
                        "Discord notification failed on retry", error=str(retry_error)
                    )
            else:
                logger.error(
