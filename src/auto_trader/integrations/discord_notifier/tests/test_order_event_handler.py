...
from auto_trader.integrations.discord_notifier.order_event_handler import (
    DiscordOrderEventHandler,
)
...
class TestDiscordOrderEventHandler:
    ...
    @pytest.mark.asyncio
    async def test_handle_order_submitted_event(
        self, event_handler, mock_notifier, sample_order
    ):
        ...
        mock_notifier.send_order_submitted.assert_called_once_with(
            order=sample_order, risk_amount=Decimal("240.00"), portfolio_risk=5.2
        )
    ...
    async def test_handle_order_filled_event(
        ...
        event = OrderEvent(
            ...
            event_data={"order": sample_order, "entry_price": 178.50},
        )
        ...
        mock_notifier.send_order_filled.assert_called_once_with(
            order=sample_order, entry_price=Decimal("178.50")
        )

    @pytest.mark.asyncio
    async def test_handle_order_cancelled_event(
        self, event_handler, mock_notifier, sample_order
    ):
        ...
        event = OrderEvent(
            ...
            event_data={"order": sample_order, "reason": "Risk limits exceeded"},
        )
        ...
        mock_notifier.send_order_cancelled.assert_called_once_with(
            order=sample_order, reason="Risk limits exceeded"
        )
    ...
    async def test_handle_order_rejected_event(
        ...
        event = OrderEvent(
            ...
            event_data={"order": sample_order, "reason": "Insufficient buying power"},
        )
    ...
    @pytest.mark.asyncio
    async def test_handle_bracket_order_placed_event(
        self, event_handler, mock_notifier, sample_order
    ):
    ...
    async def test_handle_event_with_exception(
        ...
        event = OrderEvent(
            ...
            event_type="order_submitted",
            new_status=OrderStatus.SUBMITTED,
            event_data={"order": sample_order, "risk_amount": 240.00},
        )
    ...
    @pytest.mark.asyncio
    async def test_bracket_order_event_missing_data(
        self, event_handler, mock_notifier, sample_order
    ):
    ...
    @pytest.mark.asyncio
    async def test_filled_event_without_entry_price(
        self, event_handler, mock_notifier, sample_order
    ):
        ...
        mock_notifier.send_order_filled.assert_called_once_with(
            order=sample_order, entry_price=None
        )
