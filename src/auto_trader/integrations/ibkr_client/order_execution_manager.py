...
class OrderExecutionManager:
    ...
    def __init__(
        ...
        self.ibkr_adapter = (
            IBKROrderAdapter(ibkr_client) if not simulation_mode else None
        )
    ...
    async def place_bracket_order(
        ...
        try:
            ...
            bracket = await self._create_bracket_order(
                entry_request, stop_loss_price, take_profit_price
            )
            ...
            if result.success and result.order_id:
                ...
                if bracket.parent_order.order_id:
                    self._active_orders[bracket.parent_order.order_id] = (
                        bracket.parent_order
                    )
                if bracket.stop_loss_order.order_id:
                    self._active_orders[bracket.stop_loss_order.order_id] = (
                        bracket.stop_loss_order
                    )
                if bracket.take_profit_order.order_id:
                    self._active_orders[bracket.take_profit_order.order_id] = (
                        bracket.take_profit_order
                    )

                await self.event_manager.emit_bracket_order_placed(
                    bracket, risk_validation
                )
    ...
    def _create_order_from_request(
        self, request: OrderRequest, order_type: OrderType
    ) -> Order:
    ...
    async def _handle_risk_rejection(
        self, request: OrderRequest, risk_validation
    ) -> OrderResult:
        ...
        return self.risk_validator.create_order_rejection_result(
            request, risk_validation
        )

    def _create_error_result(
        self, request: OrderRequest, error_msg: str
    ) -> OrderResult:
    ...
    def _create_error_result_for_order(
        self, order: Optional[Order], error_msg: str
    ) -> OrderResult:
    ...
    def _create_child_order(
        self, parent: Order, order_type: OrderType, price: Decimal, order_tag: str
    ) -> Order:
