...
class TestIBKRClient:
    ...
    @pytest.mark.asyncio
    async def test_detect_account_type_paper(
        self, ibkr_client, mock_ib, mock_account_summary
    ):
    ...
    @pytest.mark.asyncio
    async def test_detect_account_type_live(
        self, ibkr_client, mock_ib, mock_live_account_summary
    ):
    ...
    def test_on_connected_event(self, ibkr_client):
        ...
        with patch("auto_trader.integrations.ibkr_client.client.logger") as mock_logger:
            ...
            mock_logger.debug.assert_called_once_with(
                "IBKR connection event: connected"
            )
    ...
    def test_on_disconnected_event(self, ibkr_client):
        ...
        with patch("auto_trader.integrations.ibkr_client.client.logger") as mock_logger:
            ...
            mock_logger.warning.assert_called_once_with(
                "IBKR connection event: disconnected"
            )
            assert ibkr_client._connection_status.state == ConnectionState.DISCONNECTED
