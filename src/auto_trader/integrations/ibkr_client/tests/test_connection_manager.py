...
class TestConnectionManager:
    ...
    async def test_health_check(self, connection_manager):
        ...
        connection_manager._client.is_connected.return_value = True
        connection_manager._client.get_connection_status.return_value = (
    ...
    async def test_graceful_shutdown_with_position_closure(self, connection_manager):
        ...
        connection_manager.graceful_shutdown = (
            connection_manager._original_graceful_shutdown
        )
    ...
    async def test_graceful_shutdown_with_error(self, connection_manager):
        ...
        # Arrange
        connection_manager._client.disconnect.side_effect = Exception(
            "Disconnect failed"
        )
        # Use the original method for this test
        connection_manager.graceful_shutdown = (
    ...
    async def test_monitor_connection_reconnect_success(self, connection_manager):
        ...
        connection_manager._circuit_breaker.call_with_circuit_breaker.return_value = (
            None
        )
    ...
    def test_signal_handler_already_initiated(self, connection_manager):
        ...
        with patch("asyncio.create_task") as mock_create_task, patch.object(
            connection_manager, "graceful_shutdown", new_callable=AsyncMock
        ):
            # Act
    ...
    @pytest.mark.asyncio
    async def test_start_connection_monitoring_already_running(
