...
class TestHistoricalDataFetcher:
    ...
    @pytest.mark.asyncio
    async def test_fetch_historical_bars_success(
        self, fetcher, mock_ib_client, sample_ib_bars
    ):
    ...
    @pytest.mark.asyncio
    async def test_fetch_startup_context_with_errors(
        self, fetcher, mock_ib_client, sample_ib_bars
    ):
        ...
        )  # Fetch error + startup context error
    ...
    def test_is_market_hours_gap_weekday(self, fetcher):
        ...
        monday_open = datetime(
            2025, 8, 25, 14, 30, 0, tzinfo=UTC
        )  # 2:30 PM UTC (10:30 AM ET)
        monday_close = datetime(
            2025, 8, 25, 15, 0, 0, tzinfo=UTC
        )  # 3:00 PM UTC (11:00 AM ET)
    ...
    @pytest.mark.asyncio
    async def test_fetch_bars_for_plans(
        self, fetcher, mock_ib_client, sample_trade_plan
    ):
