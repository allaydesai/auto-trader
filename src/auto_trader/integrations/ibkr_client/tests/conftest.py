...
def mock_ib():
    ...
    with patch("auto_trader.integrations.ibkr_client.client.IB") as mock_ib_class:
        ...
        mock_ib.disconnectedEvent.__iadd__ = Mock(
            return_value=mock_ib.disconnectedEvent
        )
...
def ibkr_client(mock_settings, mock_config_loader, mock_ib):
    ...
    with patch(
        "auto_trader.integrations.ibkr_client.client.ConfigLoader", mock_config_loader
    ):
...
def circuit_breaker(temp_state_dir):
    ...
    breaker = CircuitBreaker(
        failure_threshold=3, reset_timeout=60, state_file=state_file
    )
    yield breaker
...
def connection_manager(mock_settings, temp_state_dir, mock_config_loader):
    ...
    with patch(
        "auto_trader.integrations.ibkr_client.connection_manager.IBKRClient"
    ) as mock_client_class, patch(
        "auto_trader.integrations.ibkr_client.connection_manager.CircuitBreaker"
