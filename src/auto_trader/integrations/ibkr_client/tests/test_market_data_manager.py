...
def mock_cache():
    ...
    cache.get_memory_usage = MagicMock(
        return_value={"total_bars": 100, "estimated_memory_mb": 0.1}
    )
    return cache
...
class TestMarketDataManager:
    ...
    @pytest.mark.asyncio
    async def test_subscribe_symbols_with_multiple_timeframes(
        self, manager, mock_ib_client
    ):
