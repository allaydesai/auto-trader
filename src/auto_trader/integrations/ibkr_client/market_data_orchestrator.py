...
class MarketDataOrchestrator:
    ...
    def __init__(
        self, subscription_manager: SubscriptionManager, cache: MarketDataCache
    ):
        """
    ...
    async def orchestrate_subscriptions(
        self, symbols: List[str], bar_sizes: List[BarSizeType], callback_factory
    ) -> Dict[str, bool]:
        """
    ...
    async def _subscribe_symbol_to_all_timeframes(
        self, symbol: str, bar_sizes: List[BarSizeType], callback_factory
    ) -> Dict[str, bool]:
        """
    ...
    async def orchestrate_symbol_sync(
        ...
        # Execute changes
        await self._handle_symbol_additions(
            symbols_to_add, callback_factory, default_bar_sizes
        )
        await self._handle_symbol_removals(symbols_to_remove)

    async def _handle_symbol_additions(
        self, symbols_to_add: Set[str], callback_factory, bar_sizes: List[BarSizeType]
    ) -> None:
        """
        ...
        logger.info(
            "Adding new market data subscriptions", symbols=list(symbols_to_add)
        )
    ...
    async def _handle_symbol_removals(self, symbols_to_remove: Set[str]) -> None:
        ...
        logger.info(
            "Removing unused market data subscriptions", symbols=list(symbols_to_remove)
        )
