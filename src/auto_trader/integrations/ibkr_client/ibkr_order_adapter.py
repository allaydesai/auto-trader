...
class IBKROrderAdapter:
    ...
    async def setup_event_handlers(self, status_callback: Callable) -> None:
        ...
        if hasattr(self.ibkr_client, "_ib") and self.ibkr_client._ib:
            ...
            if hasattr(ib, "orderStatusEvent") and not hasattr(
                ib.orderStatusEvent, "_mock_name"
            ):
    ...
    async def place_bracket_order(self, bracket: BracketOrder) -> OrderResult:
        ...
        try:
            ...
            # Create and qualify contract
            contract = Stock(
                bracket.parent_order.symbol, "SMART", bracket.parent_order.currency
            )
            await ib.qualifyContractsAsync(contract)
            ...
            # Create parent order
            action = (
                bracket.parent_order.side.value
                if hasattr(bracket.parent_order.side, "value")
                else str(bracket.parent_order.side)
            )
            if bracket.parent_order.order_type == OrderType.MARKET:
                parent_ib_order = MarketOrder(
                    action=action, totalQuantity=bracket.parent_order.quantity
                )
            else:
            ...
            stop_action = (
                bracket.stop_loss_order.side.value
                if hasattr(bracket.stop_loss_order.side, "value")
                else str(bracket.stop_loss_order.side)
            )
            stop_ib_order = StopOrder(
                action=stop_action,
                totalQuantity=bracket.stop_loss_order.quantity,
                stopPrice=float(bracket.stop_loss_order.stop_price)
                if bracket.stop_loss_order.stop_price
                else 0.0,
            )
            ...
            profit_action = (
                bracket.take_profit_order.side.value
                if hasattr(bracket.take_profit_order.side, "value")
                else str(bracket.take_profit_order.side)
            )
            profit_ib_order = LimitOrder(
                ...
                if bracket.take_profit_order.price
                else 0.0,
            )
            ...
            bracket.parent_order.status = self._map_ibkr_status(
                parent_trade.orderStatus.status
            )
            bracket.stop_loss_order.status = self._map_ibkr_status(
                stop_trade.orderStatus.status
            )
            bracket.take_profit_order.status = self._map_ibkr_status(
                profit_trade.orderStatus.status
            )
    ...
    async def modify_order(
        ...
        try:
            ...
            logger.info(
                ...
                order_id=order.order_id,
                new_price=float(modification.new_price)
