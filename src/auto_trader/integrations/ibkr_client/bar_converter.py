...
class BarConverter:
    ...
    def handle_bar_processing_error(
        self, error: Exception, bars: RealTimeBar, symbol: str, bar_size: BarSizeType
    ) -> None:
        """
        ...
        error_details = (
            f"Error processing bar update - {type(error).__name__}: {str(error)}"
        )

        if bars:
            error_details += f" | bars_type: {type(bars).__name__}"
            error_details += (
                f" | bars_data: {str(bars)[:200]}..."  # Limit to first 200 chars
            )
        else:
            error_details += " | bars_data: None"

        logger.error(error_details, symbol=symbol, bar_size=bar_size)

    def get_stats(self) -> dict:
        """Get bar converter statistics."""
        return {"data_quality_errors": self._data_quality_errors}
