...
class OrderEventManager:
    ...
    async def emit_order_submitted(self, order: Order, risk_validation: Any) -> None:
        ...
        event_data = {
            ...
            "risk_amount": getattr(
                risk_validation.position_size_result, "dollar_risk", None
            )
            if risk_validation
            else None,
    ...
    async def emit_bracket_order_placed(
        self, bracket: BracketOrder, risk_validation: Any
    ) -> None:
        """Emit bracket order placed event."""
        event_data = {
            "order": bracket.parent_order.model_dump(),
            "stop_loss_price": float(bracket.stop_loss_order.stop_price)
            if bracket.stop_loss_order.stop_price
            else None,
            "take_profit_price": float(bracket.take_profit_order.price)
    ...
    async def emit_order_rejected(
        self, order: Order, rejection_reasons: List[str]
    ) -> None:
    ...
    async def _emit_event(
        self, order: Order, event_type: str, event_data: Dict[str, Any]
    ) -> None:
