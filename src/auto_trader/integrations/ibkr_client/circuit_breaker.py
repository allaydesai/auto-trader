...
class CircuitBreaker:
    ...
    async def call_with_circuit_breaker(self, async_func: Callable, *args, **kwargs):
        """
        ...
        try:
            ...
            if self._state.failure_count > 0:
                ...
                logger.info(
                    f"Waiting {delay}s before retry attempt {self._state.failure_count + 1}"
                )
    ...
    def record_failure(self) -> None:
        ...
        if self._state.failure_count >= self._failure_threshold:
            ...
            self._state.next_attempt_time = datetime.now() + timedelta(
        ...
        try:
            if self._state_file.exists():
                with open(self._state_file, "r") as f:
                    ...
                    logger.debug(
                        "Circuit breaker state loaded", state=state.state.value
                    )
                    return state
        except Exception as e:
    ...
    def _save_state(self) -> None:
        ...
        try:
            ...
            if self._state.last_failure_time:
                state_dict["last_failure_time"] = (
                    self._state.last_failure_time.isoformat()
                )
            if self._state.next_attempt_time:
                state_dict["next_attempt_time"] = (
                    self._state.next_attempt_time.isoformat()
                )
