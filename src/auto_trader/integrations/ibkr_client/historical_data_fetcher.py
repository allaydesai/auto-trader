...
class HistoricalDataFetcher:
    ...
    def __init__(self, ib_client: IB):
        ...
        # Statistics
        self._stats = {"bars_fetched": 0, "fetch_errors": 0, "gaps_detected": 0}

        logger.info("HistoricalDataFetcher initialized")

    async def fetch_startup_context(
        self, symbols: List[str], bar_size: BarSizeType = "5min", duration: str = "1 D"
        ...
        for symbol in symbols:
            try:
                bars = await self.fetch_historical_bars(symbol, bar_size, duration)
                ...
                logger.error(
                    "Failed to fetch startup context", symbol=symbol, error=str(e)
                )
                context_data[symbol] = []
    ...
    def detect_data_gaps(
        self, bars: List[BarData], bar_size: BarSizeType
    ...
    def calculate_position_vs_levels(
        self, bars: List[BarData], plan: TradePlan
    ) -> Dict[str, Any]:
        """
    ...
    def _is_market_hours_gap(self, start: datetime, end: datetime) -> bool:
        """
        ...
        return (
            market_open <= start_hour <= market_close
            and market_open <= end_hour <= market_close
        )
    ...
    async def fetch_bars_for_plans(
        ...
        # Fetch historical data
        historical_data = await self.fetch_startup_context(symbols, bar_size, duration)
        ...
        for plan in plans:
            ...
            if plan.symbol not in results:
                results[plan.symbol] = {"bars": bars, "positions": {}}
