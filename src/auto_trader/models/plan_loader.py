...
class TradePlanLoader:
    ...
    def load_all_plans(self, validate: bool = True) -> Dict[str, TradePlan]:
        ...
        yaml_files = list(self.plans_directory.glob("*.yaml")) + list(
            self.plans_directory.glob("*.yml")
        )

        logger.info(
            f"Loading plans from {len(yaml_files)} files",
            directory=str(self.plans_directory),
        )
    ...
    def start_file_watching(self) -> None:
        ...
        if not self.plans_directory.exists():
            logger.warning(
                f"Cannot watch non-existent directory: {self.plans_directory}"
            )
            return
        ...
        self._observer.schedule(
            self._watcher, str(self.plans_directory), recursive=False
        )
        ...
        try:
            ...
            if not result.is_valid:
                if validate:
                    ...
                    logger.warning(
                        f"Loading file with validation warnings: {file_path}"
                    )
                ...
                try:
                    ...
                    if trade_plan.plan_id in self._loaded_plans:
                        logger.warning(
                            ...
                            existing_file=str(
                                self._plan_to_file.get(trade_plan.plan_id)
                            ),
                        )
                        continue
                    ...
                    logger.error(
                        f"Failed to create plan from data in {file_path}", error=str(e)
                    )
                    continue
