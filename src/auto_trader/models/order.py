...
from .enums import OrderType, OrderSide, OrderStatus, BracketOrderType, TimeInForce
...
class Order(BaseModel):
    ...
    parent_order_id: Optional[str] = Field(
        None, description="Parent order ID for bracket orders"
    )
    ...
    price: Optional[Decimal] = Field(
        None, gt=0, decimal_places=4, description="Limit price"
    )
    stop_price: Optional[Decimal] = Field(
        None, gt=0, decimal_places=4, description="Stop price"
    )
    trail_amount: Optional[Decimal] = Field(
        None, gt=0, decimal_places=4, description="Trailing amount"
    )
    trail_percent: Optional[Decimal] = Field(
        None, gt=0, le=100, decimal_places=2, description="Trailing percent"
    )
    ...
    submitted_at: Optional[datetime] = Field(
        None, description="When order was submitted to IBKR"
    )
    ...
    average_fill_price: Optional[Decimal] = Field(
        None, decimal_places=4, description="Average fill price"
    )

    # Commission and fees
    commission: Optional[Decimal] = Field(
        None, decimal_places=4, description="Order commission"
    )

    # Error handling
    error_message: Optional[str] = Field(
        None, description="Error message if order failed"
    )
...
class OrderRequest(BaseModel):
    ...
    stop_loss_price: Decimal = Field(
        ..., gt=0, decimal_places=4, description="Stop loss price"
    )
    take_profit_price: Decimal = Field(
        ..., gt=0, decimal_places=4, description="Take profit price"
    )

    # Risk management
    risk_category: RiskCategory = Field(
        ..., description="Risk level for position sizing"
    )
    calculated_position_size: Optional[int] = Field(
        None, gt=0, description="Calculated position size"
    )
...
class BracketOrder(BaseModel):
    ...
    bracket_type: BracketOrderType = Field(
        default=BracketOrderType.PARENT, description="Bracket type"
    )
    transmit_parent: bool = Field(
        default=False, description="Transmit parent order first"
    )

    # Status tracking
    created_at: datetime = Field(default_factory=lambda: datetime.now(UTC))
    all_orders_submitted: bool = Field(
        default=False, description="All orders submitted to IBKR"
    )
...
class OrderEvent(BaseModel):
    ...
    event_type: str = Field(
        ..., description="Event type (status_change, fill, error, etc.)"
    )
    ...
    fill_quantity: Optional[int] = Field(
        None, description="Quantity filled in this event"
    )
    fill_price: Optional[Decimal] = Field(
        None, decimal_places=4, description="Fill price"
    )
...
class OrderModification(BaseModel):
    ...
    new_price: Optional[Decimal] = Field(
        None, gt=0, decimal_places=4, description="New limit price"
    )
    new_stop_price: Optional[Decimal] = Field(
        None, gt=0, decimal_places=4, description="New stop price"
    )
    new_trail_amount: Optional[Decimal] = Field(
        None, gt=0, decimal_places=4, description="New trailing amount"
    )
    new_trail_percent: Optional[Decimal] = Field(
        None, gt=0, le=100, decimal_places=2, description="New trailing percent"
    )
