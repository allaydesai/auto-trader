...
class YAMLErrorEnhancer:
    ...
    def enhance_yaml_error(
        yaml_content: str, yaml_error: yaml.YAMLError, file_path: Optional[Path] = None
        ...
        return TradePlanValidationError(
            message=error_msg, line_number=line_number, suggestion=suggestion
        )

    @staticmethod
    def _get_error_context(
        yaml_content: str, line_number: Optional[int], column: Optional[int]
    ) -> str:
...
class ValidationReporter:
    ...
    def add_result(
        self, result: ValidationResult, file_path: Optional[Path] = None
    ) -> None:
