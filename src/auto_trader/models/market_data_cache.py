...
class MarketDataCache:
    ...
    async def update_bar(self, bar: BarData) -> None:
        ...
        with self._lock:
            ...
            if key in self._cache.bars:
                ...
                if excess > 0:
                    self._cache.bars[key] = self._cache.bars[key][
                        -self.max_bars_per_symbol :
                    ]
    ...
    def get_latest_bar(
        self, symbol: str, bar_size: BarSizeType, check_stale: bool = True
    ) -> Optional[BarData]:
    ...
    def get_bars(
        self, symbol: str, bar_size: BarSizeType, limit: Optional[int] = None
    ) -> List[BarData]:
    ...
    def is_data_stale(self, symbol: str, bar_size: BarSizeType) -> bool:
        ...
        """
        with self._lock:
            return self._cache.is_stale(symbol, bar_size, self.stale_data_multiplier)

    async def populate_historical(self, symbol: str, bars: List[BarData]) -> None:
        ...
        with self._lock:
            ...
            logger.info("Historical data populated", symbol=symbol, bar_count=len(bars))
    ...
    def remove_subscription(self, symbol: str) -> None:
        ...
        with self._lock:
            ...
            keys_to_remove = [
                key for key in self._cache.bars.keys() if key.startswith(f"{symbol}:")
            ]
    ...
    def clear_cache(self) -> None:
        ...
        with self._lock:
            ...
            logger.info("Cache cleared", bars_removed=bars_removed)
        ...
        with self._lock:
            ...
            for key, bars in self._cache.bars.items():
                if bars:
                    ...
                    if not summary["oldest_bar"] or bars[
                        0
                    ].timestamp < datetime.fromisoformat(summary["oldest_bar"]):
                        summary["oldest_bar"] = bars[0].timestamp.isoformat()
                    if not summary["newest_bar"] or bars[
                        -1
                    ].timestamp > datetime.fromisoformat(summary["newest_bar"]):
