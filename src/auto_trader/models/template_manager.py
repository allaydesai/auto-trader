...
class TemplateManager:
    ...
    def __init__(self, templates_dir: Optional[Path] = None) -> None:
        ...
        if templates_dir is None:
            ...
            project_root = Path(__file__).parents[
                3
            ]  # Go up from src/auto_trader/models/
    ...
    def list_available_templates(self) -> Dict[str, Path]:
        ...
        logger.info(
            f"Found {len(templates)} templates", templates=list(templates.keys())
        )
        return templates
    ...
    def get_template_documentation(self, template_name: str) -> Dict[str, Any]:
        ...
        for line in lines:
            ...
            # Extract examples
            if (line.startswith("# Example") and ":" in line) or line.startswith(
                "# plan_id:"
            ):
                if ":" in line:
    ...
    def customize_template(
        self, template_name: str, substitutions: Dict[str, Any], validate: bool = True
    ) -> str:
        """
        ...
        if validate:
            ...
            if not result.is_valid:
                ...
                raise ValueError(
                    f"Template customization failed validation:\n{error_summary}"
                )
    ...
    def create_plan_from_template(
        ...
        """
        # Customize template
        customized_content = self.customize_template(
            template_name, plan_data, validate=True
        )
        ...
        if output_file:
            ...
            logger.info(
                f"Saved trade plan to {output_file}", plan_id=trade_plan.plan_id
            )
            ...
            for key, value in substitutions.items():
                ...
                if "threshold:" in processed_line and key == "threshold":
                    processed_line = re.sub(
                        r"threshold: [\d.]+", f"threshold: {value}", processed_line
                    )
    ...
    def validate_template(self, template_name: str) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"Template '{template_name}' has YAML syntax issues", error=str(e)
                )
                return False
