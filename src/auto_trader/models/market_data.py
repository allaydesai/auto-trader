...
class BarData(BaseModel):
    ...
    open_price: Decimal = Field(
        ..., gt=0, decimal_places=4, description="Opening price"
    )
    ...
    close_price: Decimal = Field(
        ..., gt=0, decimal_places=4, description="Closing price"
    )
...
class MarketData(BaseModel):
    ...
    bars: Dict[str, List[BarData]] = Field(
        default_factory=dict, description="Bars indexed by 'symbol:bar_size' key"
    )
    ...
    def get_latest_bar(self, symbol: str, bar_size: BarSizeType) -> Optional[BarData]:
    ...
    def is_stale(
        self, symbol: str, bar_size: BarSizeType, max_age_multiplier: int = 2
    ) -> bool:
    ...
    def remove_old_bars(self, max_age_hours: int = 24) -> int:
        ...
        for key in list(self.bars.keys()):
            ...
            self.bars[key] = [bar for bar in self.bars[key] if bar.timestamp > cutoff]
