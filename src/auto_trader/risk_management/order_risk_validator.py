...
class OrderRiskValidator:
    ...
    async def validate_order_request(
        ...
        try:
            ...
            await self._validate_portfolio_risk_limit(position_size_result.dollar_risk)
    ...
    async def _validate_available_capital(
        ...
        try:
            ...
            position_value = (
                Decimal(str(position_size_result.position_size))
                * order_request.entry_price
            )
            required_capital = position_value * Decimal(
                "1.02"
            )  # 2% buffer for commissions/slippage
    ...
    def create_order_rejection_result(
        self, order_request: OrderRequest, validation_result: RiskValidationResult
    ) -> OrderResult:
