...
class TestPositionSizerValidation:
    ...
    def test_zero_account_value(
        ...
        with pytest.raises(
            InvalidPositionSizeError, match="Account value must be positive"
        ):
    ...
    def test_invalid_risk_category(
        ...
        with pytest.raises(
            InvalidPositionSizeError, match="Invalid risk category 'invalid'"
        ):
            position_sizer.calculate_position_size(
                account_value=sample_account_value,
    ...
    def test_zero_entry_price(
        ...
        with pytest.raises(
            InvalidPositionSizeError, match="Entry price must be positive"
        ):
            position_sizer.calculate_position_size(
                account_value=sample_account_value,
                risk_category="normal",
    ...
    def test_zero_stop_loss(
        ...
        with pytest.raises(
            InvalidPositionSizeError, match="Stop loss price must be positive"
        ):
            position_sizer.calculate_position_size(
                account_value=sample_account_value,
                risk_category="normal",
