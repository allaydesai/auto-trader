...
class TestListPlansEnhancedIntegration(TestStory153EndToEndIntegration):
    ...
    def test_list_plans_with_multiple_statuses_and_sorting(self, mock_risk_manager):
        ...
        def mock_validate(plan):
            ...
            mock_pos_result.risk_amount_percent = risk_map.get(
                plan.plan_id, Decimal("2.0")
            )
        ...
        with patch(
            "auto_trader.cli.management_commands.get_portfolio_risk_summary"
        ) as mock_portfolio:
            ...
            assert (
                "posit" in output_lower and "size" in output_lower
            ) or "position size" in output_lower
...
class TestValidateConfigIntegration(TestStory153EndToEndIntegration):
    ...
    def test_comprehensive_validation_with_portfolio_risk(self, mock_risk_manager):
        ...
        def mock_validate_file(file_path):
            ...
            return {"syntax_valid": True, "business_logic_valid": True, "errors": []}

        # Test comprehensive validation
        with patch(
            "auto_trader.cli.validation_utils.validate_all_plans"
        ) as mock_validate:
            ...
            assert "validation" in output.lower() and (
...
class TestUpdatePlanIntegration(TestStory153EndToEndIntegration):
    ...
    def test_update_multiple_fields(self, mock_risk_manager):
        ...
        mock_rm.validate_trade_plan.return_value = MagicMock(
            passed=True,
            position_size_result=MagicMock(
                position_size=50, risk_amount_dollars=Decimal("150.00")
            ),
        )
        mock_risk_manager.return_value = mock_rm
    ...
    def test_comprehensive_statistics_display(self, mock_risk_manager):
        ...
        plans = [
            ...
            (
                ...
                "position_open",
                "small",
            ),  # Same symbol, different status/risk
        ...
        with patch(
            "auto_trader.cli.management_utils.get_portfolio_risk_summary"
        ) as mock_summary:
            ...
            assert (
                "awaiting" in output.lower() and "entry" in output.lower()
...
class TestErrorHandlingAndRecovery(TestStory153EndToEndIntegration):
    ...
    def test_missing_directories_handling(self):
        ...
        assert result.exit_code in [
            ...
            1,
            2,
        ]  # Success with message, controlled error, or usage error

        # Output should indicate the issue
        assert any(
            indicator in result.output.lower()
            for indicator in ["no", "not found", "missing", "error", "directory"]
        )
    ...
    def test_corrupted_plan_files_handling(self):
        ...
        assert "no trade plans found" in output or any(
            indicator in output
            for indicator in ["error", "invalid", "warning", "failed", "corrupt"]
        )
    ...
    def test_risk_manager_failure_handling(self, mock_risk_manager):
        ...
        if output:  # If there is output, it should contain error indicators
            assert any(
                indicator in output
                for indicator in ["error", "failed", "connection", "risk"]
            )
