...
def _sort_plans_by_criteria(
    ...
    if sort_by == "risk" and risk_results:
        # Create risk lookup
        risk_lookup = {
            r["plan_id"]: r.get("risk_percent", Decimal("0")) for r in risk_results
        }
        return sorted(
            plans, key=lambda p: risk_lookup.get(p.plan_id, Decimal("0")), reverse=True
        )
    elif sort_by == "date":
        return sorted(
            plans, key=lambda p: p.plan_id, reverse=True
        )  # plan_id contains date
    elif sort_by == "symbol":
...
def process_plan_field_updates(
    plan: TradePlan,
    ...
    if risk_category is not None:
        ...
        if risk_category.lower() not in valid_categories:
            raise PlanUpdateError(
                f"Risk category must be one of: {', '.join(valid_categories)}"
            )
...
def _create_and_validate_updated_plan(
    plan: TradePlan, updates: Dict[str, Any]
) -> TradePlan:
    """
...
def _display_update_preview_and_confirmation(
    plan_id: str,
    plan: TradePlan,
...
def _perform_plan_update(
    plan_id: str, updated_plan: TradePlan, plans_dir: Path, backup_dir: Path
) -> Path:
    """
...
def _display_update_success(
    plan_id: str, updated_validation, backup_path: Path
) -> None:
    """
    ...
    if updated_validation and updated_validation.passed:
        console.print(
            f"📊 Position size recalculated: {updated_validation.position_size} shares"
        )
