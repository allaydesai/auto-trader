...
def calculate_single_plan_risk(
    plan: TradePlan,
...
def calculate_all_plan_risks(
    plans: List[TradePlan], risk_manager: RiskManager, use_cache: bool = True
) -> Dict[str, Any]:
    """
    ...
    portfolio_summary = get_portfolio_risk_summary(
        risk_results, risk_manager.portfolio_tracker
    )
    ...
    enhanced_summary = {
        **portfolio_summary,
        ...
        "capacity_utilization_percent": (current_risk / max_risk * 100)
        if max_risk > 0
        else Decimal("0"),
        "exceeds_limit": current_risk > max_risk,
        "near_limit": current_risk > (max_risk * Decimal("0.8")),
        "total_plan_risk_percent": _safe_sum_risk_percent(
            [r for r in risk_results if _should_include_in_total_risk(r)]
        ),
        "plan_risks": {
            r["plan_id"]: r
            for r in risk_results
            if "plan_id" in r and _should_include_in_total_risk(r)
        },
    }
    ...
    performance = {
        ...
        "calculations_per_second": len(plans) / execution_time
        if execution_time > 0
        else 0,
    }
...
def calculate_batch_plan_risks(
    plans: List[TradePlan], risk_manager: RiskManager, use_cache: bool = True
) -> List[Dict[str, Any]]:
    """
    ...
    for plan in plans:
        try:
            ...
            if validation_result.passed and hasattr(
                validation_result, "position_size_result"
            ):
                position_size = getattr(
                    validation_result.position_size_result, "position_size", 0
                )
                risk_amount = getattr(
                    validation_result.position_size_result,
                    "risk_amount_dollars",
                ...
                position_size = (
                    getattr(validation_result, "position_size", 0)
                    if validation_result.passed
                    else 0
                )
                risk_amount = (
                    getattr(validation_result, "risk_amount", Decimal("0"))
                    if validation_result.passed
                    else Decimal("0")
                )
                risk_percent = (
                    getattr(validation_result, "risk_percent", Decimal("0"))
                    if validation_result.passed
                    else Decimal("0")
                )
...
def get_portfolio_risk_summary(
    risk_results: List[Dict[str, Any]], portfolio_tracker: PortfolioTracker
) -> Dict[str, Any]:
    """
