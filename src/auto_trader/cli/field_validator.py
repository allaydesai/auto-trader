...
class WizardFieldValidator:
    ...
    def validate_stop_loss(
        self, stop_loss: Decimal, entry_level: Decimal
    ) -> Dict[str, Any]:
        """
        ...
        if stop_loss == entry_level:
            return {"is_valid": False, "error": "Stop loss cannot equal entry level"}
    ...
    def _get_user_friendly_error(
        self, field_name: str, value: Any, original_error: str
    ) -> str:
        """
        ...
        if field_name == "symbol":
            ...
            try:
                ...
                if decimal_value <= 0:
                    return (
                        field_messages["negative"]
                        if decimal_value < 0
                        else field_messages["zero"]
                    )
    ...
    def _create_dummy_data_for_field(
        ...
        if field_name == "entry_level" and isinstance(value, Decimal):
            ...
            dummy_data["stop_loss"] = round(
                value * Decimal("0.95"), 4
            )  # 5% below entry
            dummy_data["take_profit"] = round(
                value * Decimal("1.05"), 4
            )  # 5% above entry

        elif field_name == "stop_loss" and isinstance(value, Decimal):
            # Adjust entry and target to be valid relative to stop
            if value > Decimal("0"):
                dummy_data["entry_level"] = round(
                    value * Decimal("1.05"), 4
                )  # Entry above stop
                dummy_data["take_profit"] = round(
                    value * Decimal("1.15"), 4
                )  # Target above entry
