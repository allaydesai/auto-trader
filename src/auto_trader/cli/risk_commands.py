...
def calculate_position_size(
    ...
    try:
        logger.info(
            "Position size calculation started",
            symbol=symbol,
            entry=entry,
            stop=stop,
            risk=risk,
...
def _display_portfolio_summary(summary: dict, account_value: float) -> None:
    ...
    overview_table.add_row(
        "📊 Current Risk:", f"{summary['current_risk_percentage']:.1f}%"
    )
    overview_table.add_row("🔒 Risk Limit:", f"{summary['risk_limit']:.1f}%")
    overview_table.add_row(
        "💡 Available Capacity:", f"{summary['remaining_capacity_percent']:.1f}%"
    )
    overview_table.add_row(
        "💵 Available Dollars:", f"${summary['remaining_capacity_dollars']:,.2f}"
    )
