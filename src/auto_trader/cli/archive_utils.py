...
def organize_plans_for_archive(plans: List[TradePlan]) -> Dict[str, List[TradePlan]]:
    ...
    """
    archive_groups = {"completed": [], "cancelled": [], "error": []}
...
def create_archive_preview_table(
    archive_groups: Dict[str, List[TradePlan]],
) -> tuple[Table, int]:
    """
    ...
    for category, plans in archive_groups.items():
        ...
        if len(plan_ids) > 3:
            plan_display = (
                f"{', '.join(plan_ids[:3])}, ... and {len(plan_ids) - 3} more"
            )
        else:
...
def perform_plan_archiving(
    archive_groups: Dict[str, List[TradePlan]], plans_dir: Path, archive_base_dir: Path
) -> Dict[str, int]:
    """
...
def get_archive_statistics(archive_base_dir: Path) -> Dict[str, any]:
        ...
        for year_dir in category_dir.rglob("*.yaml"):
            ...
            try:
                ...
                if len(parts) >= 3:
                    ...
                    stats["by_month"][year_month] = (
                        stats["by_month"].get(year_month, 0) + 1
                    )
    ...
    if archive_files:
        ...
        stats["oldest_archive"] = datetime.fromtimestamp(
            archive_files[0].stat().st_ctime
        )
        stats["newest_archive"] = datetime.fromtimestamp(
            archive_files[-1].stat().st_ctime
        )
...
def restore_plan_from_archive(
    plan_id: str, archive_base_dir: Path, plans_dir: Path
) -> bool:
    """
