...
def list_plans_enhanced(
    plans_dir: Optional[Path], status: Optional[str], sort_by: str, verbose: bool
) -> None:
    ...
    try:
        ...
        _display_plans_listing_output(
            plans, plan_risk_data, portfolio_data, verbose, status
        )
...
def validate_config(
    plans_dir: Optional[Path], file: Optional[Path], verbose: bool
) -> None:
    """Comprehensive validation of trade plan configuration with UX compliance."""
    logger.info(
        "Comprehensive validation started", single_file=str(file) if file else None
    )
    ...
    try:
        ...
        _display_validation_guidance(
            results["file_results"], results["portfolio_risk_passed"]
        )
        ...
        console.print(f"[red]File system error: {e}[/red]")
        logger.error(
            "Comprehensive validation failed - file system error", error=str(e)
        )
...
def update_plan(
    ...
    try:
        ...
        updates = process_plan_field_updates(
            plan, entry_level, stop_loss, take_profit, risk_category
        )
        ...
        updated_plan, original_validation, updated_validation = (
            _prepare_plan_update_data(plan, updates, risk_manager)
        )
        ...
        console.print(f"[red]File system error: {e}[/red]")
        logger.error(
            "Plan update failed - file system error", plan_id=plan_id, error=str(e)
        )
        raise click.ClickException("Failed to access plan files")
    except PlanLoadingError as e:
        console.print(f"[red]Plan loading error: {e}[/red]")
        ...
        logger.error(
            "Plan update failed - backup creation", plan_id=plan_id, error=str(e)
        )
        ...
        logger.error(
            "Plan update failed - backup verification", plan_id=plan_id, error=str(e)
        )
        raise click.ClickException("Backup verification failed, plan update cancelled")
    except RiskCalculationError as e:
        console.print(f"[red]Risk calculation error: {e}[/red]")
        logger.error(
            "Plan update failed - risk calculation", plan_id=plan_id, error=str(e)
        )
        raise click.ClickException("Failed to calculate plan risks")
    except PlanManagementError as e:
        console.print(f"[red]Plan management error: {e}[/red]")
        logger.error(
            "Plan update failed - management error", plan_id=plan_id, error=str(e)
        )
...
@click.option(
    "--dry-run", is_flag=True, help="Show what would be archived without moving files"
)
@click.option("--force", is_flag=True, help="Skip confirmation prompts")
def archive_plans(
    plans_dir: Optional[Path],
...
def plan_stats(plans_dir: Optional[Path]) -> None:
    ...
    try:
        ...
        status_counts, symbol_counts, risk_counts = _calculate_plan_statistics(
            all_plans
        )
        ...
        _display_statistics_output(
            portfolio_data, status_counts, symbol_counts, risk_counts, total_plans
        )
        _log_statistics_completion(
            total_plans, symbol_counts, portfolio_data, risk_results
        )
