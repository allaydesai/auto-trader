...
def validate_single_file(
    file_path: Path, validation_engine: ValidationEngine
) -> Dict[str, Any]:
    """
...
def validate_all_plans(
    ...
    results["business_logic_passed"] = (
        results["total_files"] - results["business_logic_errors"]
    )
...
def check_portfolio_risk_compliance(
    plans_dir: Path, portfolio_tracker: PortfolioTracker
) -> Dict[str, Any]:
    """
...
def create_validation_summary_table(results: Dict[str, Any]) -> Table:
    ...
    table.add_row(
        "YAML Syntax", syntax_status, f"{syntax_passed}/{results['total_files']} files"
    )
    ...
    table.add_row(
        "Business Logic", logic_status, f"{logic_passed}/{results['total_files']} files"
    )

    return table


def create_validation_details_table(
    file_results: Dict[str, Dict], verbose: bool = False
) -> Optional[Table]:
    """
...
def format_validation_guidance(
    file_results: Dict[str, Dict], portfolio_passed: bool
) -> str:
    """
...
def _display_validation_file_details(
    file_results: Dict[str, Dict], verbose: bool
) -> None:
...
def _display_validation_guidance(
    file_results: Dict[str, Dict], portfolio_passed: bool
) -> None:
