...
def _load_and_filter_plans(
    loader: TradePlanLoader, status: Optional[str]
) -> List[TradePlan]:
...
def _display_plans_listing_output(
    plans: List[TradePlan],
    plan_risk_data: Dict[str, Dict],
    portfolio_data: Dict,
    verbose: bool,
    status: Optional[str],
) -> None:
    ...
    risk_data_list = (
        list(plan_risk_data.values())
        if isinstance(plan_risk_data, dict)
        else plan_risk_data
    )
...
def _log_plans_listing_completion(
    plans: List[TradePlan], portfolio_data: Dict, risk_results: Dict
) -> None:
...
def _setup_update_directories(
    plans_dir: Optional[Path], backup_dir: Optional[Path]
) -> tuple[Path, Path]:
...
def _load_and_validate_plan_for_update(
    plan_id: str, loader: TradePlanLoader
) -> Optional[TradePlan]:
...
def _prepare_plan_update_data(
    plan: TradePlan, updates: Dict, risk_manager: RiskManager
) -> tuple:
...
def _setup_archive_directories(
    plans_dir: Optional[Path], archive_dir: Optional[Path]
) -> tuple[Path, Path]:
...
def _get_archive_confirmation(total_plans: int, archive_dir: Path, force: bool) -> bool:
    ...
    confirm = click.confirm(
        f"Archive {total_plans} plans to {archive_dir}?", default=False
    )
    if not confirm:
...
def _display_statistics_output(
    portfolio_data: Dict, status_counts, symbol_counts, risk_counts, total_plans: int
) -> None:
    ...
    status_table = create_statistics_overview_table(
...
def _log_statistics_completion(
    total_plans: int, symbol_counts, portfolio_data: Dict, risk_results: Dict
) -> None:
    ...
    def error_handler(func):
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                logger.error(
                    f"{operation_name} failed - file system error{extra_context}",
                    error=str(e),
                )
                ...
                logger.error(
                    f"{operation_name} failed - plan loading{extra_context}",
                    error=str(e),
                )
                ...
                logger.error(
                    f"{operation_name} failed - backup operation{extra_context}",
                    error=str(e),
                )
                ...
                logger.error(
                    f"{operation_name} failed - risk calculation{extra_context}",
                    error=str(e),
                )
                ...
                logger.error(
                    f"{operation_name} failed - operation error{extra_context}",
                    error=str(e),
                )
