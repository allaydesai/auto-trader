...
def validate_config(
    ...
    user_config_file: Optional[Path],
    verbose: bool
) -> None:
...
def setup(output_dir: Path, force: bool) -> None:
    ...
    try:
        ...
        existing_files = [
            f for f in [env_file, config_file, user_config_file] 
            if f.exists()
        ]
...
def _display_config_summary(config_loader: ConfigLoader) -> None:
    ...
    system_table.add_row("Market Hours Only", str(system_config.trading.market_hours_only))
    system_table.add_row("Default Timeframe", system_config.trading.default_timeframe)
    system_table.add_row("Max Position %", f"{system_config.risk.max_position_percent}%")
    system_table.add_row("Daily Loss Limit %", f"{system_config.risk.daily_loss_limit_percent}%")
    system_table.add_row("Max Open Positions", str(system_config.risk.max_open_positions))
    ...
    user_table.add_row("Default Account Value", f"${user_preferences.default_account_value:,}")
    user_table.add_row("Risk Category", user_preferences.default_risk_category)
    user_table.add_row("Preferred Timeframes", ", ".join(user_preferences.preferred_timeframes))
