...
def format_plan_status(status: TradePlanStatus) -> str:
    ...
    return status_formats.get(
        status, f"❓ {status.value if hasattr(status, 'value') else status}"
    )


def create_plans_listing_table(
    ...
    for plan in plans:
        row_data = [plan.plan_id, plan.symbol, format_plan_status(plan.status)]
        ...
        if show_risk:
            ...
            if "error" in risk_data:
                ...
                row_data.extend([risk_display, f"{position_size} shares"])
            else:
        ...
        if verbose:
            row_data.extend(
                [
                    ...
                    plan.risk_category.value
                    if hasattr(plan, "risk_category")
                    and hasattr(plan.risk_category, "value")
                    else (
...
def create_plan_update_preview_table(
    plan_id: str, plan: TradePlan, updates: Dict[str, Any]
) -> Table:
    """
    ...
    for field_key, new_value in updates.items():
        if hasattr(plan, field_key):
            ...
            table.add_row(field_display, formatter(current_value), formatter(new_value))
...
def display_plan_risk_impact(original_validation, updated_validation) -> None:
    ...
    # Calculate changes
    position_change = (
        updated_validation.position_size - original_validation.position_size
    )
...
Portfolio Risk: {original_validation.risk_percent:.2f}% → {updated_validation.risk_percent:.2f}% ({risk_indicator})"""
    ...
    panel = Panel(content, title="🛡️ Risk Impact Analysis", border_style=risk_style)

    console.print(panel)


def create_statistics_overview_table(
    ...
    for status, count in status_counts.items():
        ...
        status_display = format_plan_status(TradePlanStatus(status)).split(" ", 1)[
            0
        ]  # Get emoji only
        table.add_row(
            f"{status_display} {status.replace('_', ' ').title()}",
            str(count),
            f"{percentage:.1f}%",
        )
    ...
    if risk_counts:
        ...
        for risk_category, count in risk_counts.items():
            ...
            table.add_row(
                f"{risk_emoji} {risk_category.title()} Risk",
                str(count),
                f"{percentage:.1f}%",
            )
...
def display_plan_insights(
    total_plans: int, symbol_counts: Dict[str, int], portfolio_data: Dict[str, Any]
) -> None:
    """
    ...
    if risk_utilization < 30:
        insights.append(
            f"📈 Conservative risk utilization ({risk_utilization:.1f}%) - room for growth"
        )
    elif risk_utilization > 90:
        insights.append(
            f"⚠️ High risk utilization ({risk_utilization:.1f}%) - approaching limit"
        )
    else:
    ...
    if plans_with_errors > 0:
        insights.append(
            f"🔧 {plans_with_errors} plan(s) need attention - check validation"
        )

    if insights:
        content = "\n".join([f"• {insight}" for insight in insights])
        panel = Panel(content, title="💡 Portfolio Insights", border_style="blue")
        console.print(panel)
