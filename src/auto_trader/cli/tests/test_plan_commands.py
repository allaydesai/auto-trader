...
from auto_trader.cli.plan_commands import (
    validate_plans,
    list_plans,
    create_plan,
...
class TestValidatePlans:
    ...
    def test_validate_plans_verbose(self):
        ...
        with patch(
            ...
            "auto_trader.cli.plan_commands.display_plans_summary"
        ) as mock_display:
            mock_loader = MagicMock()
            mock_loader.plans_directory = Path("data/trade_plans")
            mock_loader.load_all_plans.return_value = {"plan1": MagicMock()}
    ...
    def test_validate_plans_with_watch(self):
        ...
        with patch(
            ...
            "auto_trader.cli.plan_commands.start_file_watching"
        ) as mock_watch:
            mock_loader = MagicMock()
            mock_loader.plans_directory = Path("data/trade_plans")
            mock_loader.load_all_plans.return_value = {"plan1": MagicMock()}
...
class TestListPlans:
    ...
    def test_list_plans_with_filters(self):
        ...
        with patch(
            ...
            mock_plan2 = self.create_mock_plan(
                "PLAN2", "MSFT", "position_open", "small"
            )
    ...
    def test_list_plans_sorting(self):
        ...
        with patch(
            ...
            result = runner.invoke(
                list_plans, ["--sort-by", "symbol", "--sort-desc", "--limit", "10"]
            )
...
class TestCreatePlan:
    ...
    def test_create_plan_success(self):
        ...
        with patch(
            "auto_trader.cli.plan_commands.TemplateManager"
        ) as mock_template_class, patch(
            "auto_trader.cli.plan_commands.show_available_templates"
        ) as mock_show_templates, patch(
            "auto_trader.cli.plan_commands.get_template_choice"
        ) as mock_get_choice, patch(
            "auto_trader.cli.plan_commands.get_plan_data_interactive"
        ) as mock_get_data, patch(
            "auto_trader.cli.plan_commands.create_plan_output_file"
        ) as mock_create_output, patch(
            "auto_trader.cli.plan_commands.show_plan_creation_success"
            ...
            mock_template_manager.create_plan_from_template.return_value = (
                mock_trade_plan
            )
...
class TestCreatePlanInteractive:
    ...
    def test_create_plan_interactive_success(self):
        ...
        with patch("config.ConfigLoader") as mock_config_class, patch(
            "auto_trader.risk_management.RiskManager"
        ) as mock_risk_class, patch(
            "auto_trader.cli.wizard_utils.Prompt.ask"
        ) as mock_prompt, patch(
            "auto_trader.cli.wizard_preview.TradePlanPreview.show_preview"
        ) as mock_preview, patch(
            "auto_trader.cli.wizard_plan_utils.save_plan_to_yaml"
        ) as mock_save:
            # Setup mocks
    ...
    def test_create_plan_interactive_portfolio_risk_exceeded(self):
        ...
        with patch("config.ConfigLoader") as mock_config_class, patch(
            ...
            "auto_trader.cli.wizard_utils.Confirm.ask"
        ) as mock_confirm:
            # Setup config loader
            mock_config_class.return_value = self.create_mock_config_loader()
            ...
            mock_prompt.side_effect = [
                ...
                "180.50",
                "178.00",
                "large",  # Large risk category triggers limit
            ]
    ...
    def test_create_plan_interactive_keyboard_interrupt(self):
        ...
        with patch("config.ConfigLoader") as mock_config_class, patch(
            ...
            "auto_trader.cli.wizard_utils.WizardFieldCollector.collect_symbol",
    ...
    def test_create_plan_interactive_output_directory(self):
        ...
        with patch("config.ConfigLoader") as mock_config_class, patch(
            ...
            result = self.runner.invoke(
                create_plan_interactive, ["--output-dir", "custom/plans"]
            )
    ...
    def test_create_plan_interactive_full_integration(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch("config.ConfigLoader") as mock_config_class, patch(
                ...
                "auto_trader.cli.wizard_plan_utils.datetime"
            ) as mock_datetime:
                # Setup mocks
                mock_config_class.return_value = self.create_mock_config_loader()
                mock_risk_class.return_value = self.create_mock_risk_manager()
                ...
                result = self.runner.invoke(
                    create_plan_interactive, ["--output-dir", str(temp_path)]
                )
