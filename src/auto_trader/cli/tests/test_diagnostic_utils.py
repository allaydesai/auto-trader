...
class TestDiagnosticUtils:
    ...
    def test_check_configuration_success(self, tmp_path):
        ...
        with patch("auto_trader.cli.diagnostic_utils.Settings") as mock_settings, patch(
            "auto_trader.cli.diagnostic_utils.ConfigLoader"
        ) as mock_loader, patch("auto_trader.cli.diagnostic_utils.Path") as mock_path:
            # Setup mocks
            ...
            assert any(
                r["check"] == "Environment file" and r["status"] == "✅"
                for r in results
            )
            assert any(
                r["check"] == "Configuration validation" and r["status"] == "✅"
                for r in results
            )
    ...
    def test_check_configuration_missing_env(self):
        ...
        with patch("auto_trader.cli.diagnostic_utils.Settings") as mock_settings, patch(
            ...
            assert any(
                r["check"] == "Environment file" and r["status"] == "⚠️" for r in results
            )
    ...
    def test_check_trade_plans_success(self, tmp_path):
        ...
        with patch("auto_trader.cli.diagnostic_utils.TradePlanLoader") as mock_loader:
            ...
            mock_plans_dir.glob.return_value = [
                tmp_path / "plan1.yaml",
                tmp_path / "plan2.yaml",
            ]
            mock_instance.plans_directory = mock_plans_dir
            mock_instance.load_all_plans.return_value = {
                "plan1": Mock(),
                "plan2": Mock(),
            }
            ...
            assert any(
                r["check"] == "Plans directory" and r["status"] == "✅" for r in results
            )
            assert any(
                r["check"] == "YAML files" and r["status"] == "📄" for r in results
            )
            assert any(
                r["check"] == "Plan loading" and r["status"] == "✅" for r in results
            )
