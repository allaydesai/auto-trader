...
class TestWizardFieldCollector:
    ...
    @patch("auto_trader.cli.wizard_utils.Prompt.ask")
    def test_collect_stop_loss_equal_to_entry_rejected(
        self, mock_prompt, field_collector
    ):
    ...
    def test_calculate_and_display_position_size(self, field_collector):
        ...
        field_collector.risk_manager.check_portfolio_risk_limit.return_value = (
            mock_portfolio_check
        )

        position_size, dollar_risk = (
            field_collector.calculate_and_display_position_size(
                Decimal("180.50"), Decimal("178.00"), RiskCategory.NORMAL
            )
        )
    ...
    def test_calculate_position_size_risk_limit_exceeded(self, field_collector):
        ...
        with patch("auto_trader.cli.wizard_utils.Confirm.ask", return_value=True):
            position_size, dollar_risk = (
                field_collector.calculate_and_display_position_size(
                    Decimal("180.50"), Decimal("178.00"), RiskCategory.LARGE
                )
            )
    ...
    def test_calculate_position_size_calculation_exception(self, field_collector):
        ...
        field_collector.risk_manager.position_sizer.calculate_position_size.side_effect = Exception(
            "Calculation failed"
        )
    ...
    def test_collect_execution_functions(self, mock_prompt, field_collector):
        ...
        mock_prompt.side_effect = [
            "close_above",
            "15min",  # entry function
            "close_below",
            "15min",  # stop loss function
            "close_above",
            "15min",  # take profit function
        ]

        entry_func, stop_loss_func, take_profit_func = (
            field_collector.collect_execution_functions()
        )
...
class TestTradePlanPreview:
    ...
    def sample_plan_data(self):
        ...
        return {
            ...
            "entry_function": ExecutionFunction(
                function_type="close_above", timeframe="15min"
            ),
    ...
    @patch("auto_trader.cli.wizard_preview.Prompt.ask")
    def test_show_preview_modify_then_confirm(
        self, mock_prompt, preview_manager, sample_plan_data
    ):
...
class TestIntegrationScenarios:
    ...
    def test_complete_wizard_flow_valid_inputs(
        self, sample_config_loader, sample_risk_manager
    ):
        """Test complete wizard flow with all valid inputs."""
        field_collector = WizardFieldCollector(
            sample_config_loader, sample_risk_manager
        )
        ...
        with patch.multiple(
            ...
            sample_risk_manager.position_sizer.calculate_position_size.return_value = (
            ...
            sample_risk_manager.check_portfolio_risk_limit.return_value = (
                mock_portfolio_check
            )
            ...
            position_size, dollar_risk = (
                field_collector.calculate_and_display_position_size(entry, stop, risk)
            )
    ...
    def test_cli_shortcuts_prepopulation(
        self, sample_config_loader, sample_risk_manager
    ):
