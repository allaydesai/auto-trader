...
class TestPortfolioRiskSummary:
    ...
    def test_get_portfolio_risk_summary(self, sample_plan, mock_risk_manager):
        ...
        assert portfolio_summary["capacity_utilization_percent"] == Decimal(
            "52.00"
        )  # Fixed: (5.2/10.0)*100
    ...
    def test_get_portfolio_risk_summary_near_limit(
        self, sample_plan, mock_risk_manager
    ):
        """Test portfolio risk summary when near limit."""
        mock_risk_manager.portfolio_tracker.get_current_portfolio_risk.return_value = (
            Decimal("8.5")
        )
    ...
    def test_get_portfolio_risk_summary_over_limit(
        self, sample_plan, mock_risk_manager
    ):
...
class TestComprehensiveValidation:
    ...
    def test_validate_plans_comprehensive_success(
        self, mock_loader_class, temp_dir, mock_risk_manager
    ):
    ...
    @patch("auto_trader.cli.validation_utils.TradePlanLoader")
    def test_validate_plans_comprehensive_syntax_error(
        self, mock_loader_class, temp_dir, mock_risk_manager
    ):
    ...
    def test_validate_plans_comprehensive_single_file(
        self, temp_dir, mock_risk_manager
    ):
...
class TestImprovedErrorHandling:
    ...
    @pytest.mark.skipif(
        os.name == "nt", reason="Permission testing not reliable on Windows"
    )
    ...
    def test_verify_backup_invalid_yaml(self, temp_dir):
        ...
        backup_file.write_text(
            "valid: [bad"
        )  # Invalid YAML, same length as "valid: yaml"
    ...
    def test_verify_backup_empty_file(self, temp_dir):
        ...
        original_file.write_text(
            "   "
        )  # Only whitespace, will be considered empty after strip()
    ...
    @patch("auto_trader.cli.management_utils.create_plan_backup")
    def test_perform_plan_update_backup_verification_fails(
        self, mock_backup, mock_verify, temp_dir
    ):
    ...
    @patch("auto_trader.cli.management_utils.verify_backup")
    @patch("auto_trader.cli.management_utils.create_plan_backup")
    def test_perform_plan_update_yaml_write_fails(
        self, mock_backup, mock_verify, temp_dir
    ):
    ...
    def test_perform_plan_update_success(self, mock_backup, mock_verify, temp_dir):
        ...
        result_backup_path = _perform_plan_update(
            "TEST_001", mock_plan, temp_dir, temp_dir
        )
...
class TestCalculateAllPlanRisks:
    ...
    def test_calculate_all_plan_risks_utilization_percent_accuracy(
        self, sample_plan, mock_risk_manager
    ):
    ...
    def test_calculate_all_plan_risks_near_limit_logic(
        self, sample_plan, mock_risk_manager
    ):
    ...
    def test_calculate_all_plan_risks_exceeds_limit_logic(
        self, sample_plan, mock_risk_manager
    ):
        ...
        assert portfolio_summary["remaining_capacity_percent"] == Decimal(
            "0.0"
        )  # Clamped to 0
    ...
    def test_calculate_all_plan_risks_multiple_plans_aggregation(self):
        ...
        open_plan = TradePlan(
            ...
            entry_function={
                "function_type": "close_above",
                "parameters": {"threshold": "320.00"},
                "timeframe": "15min",
            },
            stop_loss_function={
                "function_type": "close_below",
                "parameters": {"threshold": "315.00"},
                "timeframe": "1min",
            },
            take_profit_function={
                "function_type": "close_above",
                "parameters": {"threshold": "330.00"},
                "timeframe": "1min",
            },
        )
        ...
        completed_plan = TradePlan(
            ...
            stop_loss_function={
                "function_type": "close_below",
                "parameters": {"threshold": "245.00"},
                "timeframe": "1min",
            },
            take_profit_function={
                "function_type": "close_above",
                "parameters": {"threshold": "260.00"},
                "timeframe": "1min",
            },
        )
        ...
        def mock_validate_trade_plan(plan):
            ...
            if plan.plan_id == "AAPL_20250825_001":
                validation_result.position_size_result.risk_amount_percent = Decimal(
                    "2.5"
                )
                validation_result.position_size_result.position_size = 100
                validation_result.position_size_result.risk_amount_dollars = Decimal(
                    "250.0"
                )
        ...
        expected_plan_risk_sum = Decimal("2.5") + Decimal(
            "1.8"
        )  # AAPL + MSFT, not TSLA
        ...
        assert (
            "TSLA_20250825_003" not in portfolio_summary["plan_risks"]
    ...
    def test_calculate_all_plan_risks_performance_metrics(
        self, sample_plan, mock_risk_manager
    ):
        ...
        assert (
            performance["execution_time_seconds"] < 1.0
        )  # Should be sub-second for 5 mock operations
