...
@click.option(
    ...
    "-w",
    is_flag=True,
    help="Watch for file changes and validate automatically",
)
...
@click.option(
    "--status", help="Filter by status (awaiting_entry, position_open, completed)"
)
...
@click.option(
    "--sort-by",
    default="plan_id",
    type=click.Choice(["plan_id", "symbol", "status", "created_at"]),
    help="Sort plans by field",
)
...
@click.option(
    "--show-risk", is_flag=True, help="Show position sizes and risk calculations"
)
def list_plans(
    ...
    try:
        ...
        if symbol:
            filtered_plans = [
                p for p in filtered_plans if p.symbol.upper() == symbol.upper()
            ]

        if risk_category:
            filtered_plans = [
                p for p in filtered_plans if p.risk_category == risk_category
            ]
...
@click.option(
    ...
    help="Output directory for created plan",
)
def create_plan(output_dir: Optional[Path] = None) -> None:
    ...
    try:
        ...
        trade_plan = template_manager.create_plan_from_template(
            template_name, plan_data, output_file
        )
...
def create_plan_interactive(
    ...
    try:
        ...
        console.print(
            "[dim]Follow the prompts to create a new trade plan with real-time validation[/dim]\n"
        )
        ...
        position_size, dollar_risk = (
            field_collector.calculate_and_display_position_size(
                collected_entry, collected_stop, collected_risk
            )
        )
