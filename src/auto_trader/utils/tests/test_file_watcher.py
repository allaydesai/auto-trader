...
class TestTradeplanFileHandler:
    ...
    def test_event_queuing_and_debouncing(self):
        ...
        event = FileWatchEvent(
            FileWatchEventType.MODIFIED, Path("test.yaml"), time.time()
        )
...
class TestFileWatcher:
    ...
    def test_file_validation(self, temp_watch_dir):
        ...
        plan_data = {
            ...
            "take_profit": 160.00,
            "position_size": 100,
            "entry_function": {"type": "close_above", "timeframe": "15min"},
        ...
        with patch.object(watcher.validation_engine, "validate_file") as mock_validate:
            ...
            mock_validate.return_value = ValidationResult(
                is_valid=True, errors=[], warnings=[], plan_ids={"TEST_001"}
            )
    ...
    def test_file_deletion_handling(self, temp_watch_dir):
        ...
        with patch.object(
            watcher.trade_plan_loader, "clear_cache_for_file"
        ) as mock_clear:
    ...
    def test_event_handling(self, temp_watch_dir, mock_validation_callback):
        ...
        watcher = FileWatcher(
            temp_watch_dir, validation_callback=mock_validation_callback
        )
        ...
        create_event = FileWatchEvent(
            FileWatchEventType.CREATED, test_file, time.time()
        )

        with patch.object(watcher, "_validate_file") as mock_validate:
            watcher._handle_file_event(create_event)
            mock_validate.assert_called_once_with(test_file)
            mock_validation_callback.assert_called_once_with(
                test_file, FileWatchEventType.CREATED
            )
        ...
        delete_event = FileWatchEvent(
            FileWatchEventType.DELETED, test_file, time.time()
        )
        ...
        with patch.object(watcher, "_handle_file_deletion") as mock_delete:
            ...
            mock_validation_callback.assert_called_once_with(
                test_file, FileWatchEventType.DELETED
            )
...
class TestFileWatchEvent:
    ...
    def test_event_creation(self):
        ...
        event = FileWatchEvent(FileWatchEventType.MODIFIED, file_path, timestamp)
