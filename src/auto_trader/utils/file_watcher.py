...
class TradeplanFileHandler(FileSystemEventHandler):
    ...
    def __init__(
        self, callback: Callable[[FileWatchEvent], None], debounce_delay: float = 0.5
    ):
        """
    ...
    def on_created(self, event):
        ...
        if not event.is_directory and self._is_yaml_file(event.src_path):
            self._queue_event(
                FileWatchEvent(
                    FileWatchEventType.CREATED, Path(event.src_path), time.time()
                )
            )
    ...
    def on_modified(self, event):
        ...
        if not event.is_directory and self._is_yaml_file(event.src_path):
            self._queue_event(
                FileWatchEvent(
                    FileWatchEventType.MODIFIED, Path(event.src_path), time.time()
                )
            )
    ...
    def on_deleted(self, event):
        ...
        if not event.is_directory and self._is_yaml_file(event.src_path):
            self._queue_event(
                FileWatchEvent(
                    FileWatchEventType.DELETED, Path(event.src_path), time.time()
                )
            )
    ...
    def _queue_event(self, event: FileWatchEvent):
        ...
        try:
            ...
            self._debounce_tasks[key] = loop.call_later(
                self.debounce_delay, self._process_debounced_event, key
            )
        except RuntimeError:
...
class FileWatcher:
    ...
    def __init__(
        ...
        validation_callback: Optional[
            Callable[[Path, FileWatchEventType], None]
        ] = None,
    ...
    def start(self) -> bool:
        ...
        try:
            ...
            self.event_handler = TradeplanFileHandler(
                callback=self._handle_file_event, debounce_delay=self.debounce_delay
            )

            # Create observer
            self.observer = Observer()
            self.observer.schedule(
                self.event_handler, path=str(self.watch_directory), recursive=True
            )
    ...
    def _handle_file_event(self, event: FileWatchEvent):
        ...
        try:
            ...
            logger.error(
                f"Error handling file event: {e}", file_path=str(event.file_path)
            )
    ...
    def force_validation(self):
        ...
        yaml_files = list(self.watch_directory.glob("*.yaml")) + list(
            self.watch_directory.glob("*.yml")
        )
    ...
    def from_settings(cls, settings_dict: Dict[str, any]) -> "FileWatchConfig":
        ...
        return cls(
            watch_directory=Path(
                settings_dict.get("watch_directory", "data/trade_plans")
            ),
