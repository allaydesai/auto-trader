...
class OrderRequestBuilder:
    ...
    def create_exit_order(
        ...
        try:
            ...
            plan_id = (
                f"EXIT_{function_name}_{datetime.now(UTC).strftime('%Y%m%d_%H%M%S')}"
            )
            order_request = OrderRequest(
                trade_plan_id=plan_id,
                ...
                stop_loss_price=(position.current_price * Decimal("0.95")).quantize(
                    Decimal("0.0001")
                ),
                take_profit_price=(position.current_price * Decimal("1.05")).quantize(
                    Decimal("0.0001")
                ),
                risk_category=self.default_risk_category,
                calculated_position_size=abs(
                    position.quantity
    ...
    def create_stop_modification(
        ...
        try:
            ...
            plan_id = (
                f"STOP_{function_name}_{datetime.now(UTC).strftime('%Y%m%d_%H%M%S')}"
            )
            ...
            order_request = OrderRequest(
                ...
                or (position.current_price * Decimal("1.10")).quantize(
                    Decimal("0.0001")
                ),
                risk_category=self.default_risk_category,
            ...
            logger.debug(
                "Created stop modification order request",
                symbol=context.symbol,
