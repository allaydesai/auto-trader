...
class EdgeCaseDetector:
    ...
    def detect_gap(
        self, current_bar: "BarData", previous_bar: "BarData"
    ) -> EdgeCaseResult:
        ...
        if gap_percent >= self.gap_threshold_percent:
            gap_direction = (
                "up" if current_bar.open_price > previous_bar.close_price else "down"
            )
    ...
    def detect_limit_move(
        ...
        high_move = (
            (current_bar.high_price - reference_close) / reference_close
        ) * Decimal("100")
        low_move = (
            (reference_close - current_bar.low_price) / reference_close
        ) * Decimal("100")
    ...
    def detect_volume_anomaly(
        self, current_bar: "BarData", historical_bars: List["BarData"]
    ) -> EdgeCaseResult:
        ...
        avg_volume = Decimal(
            str(sum(bar.volume for bar in historical_bars[-20:]))
        ) / Decimal("20")
