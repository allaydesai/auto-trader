    ...
    async def handle_execution_signal(
        ...
        try:
            ...
            if signal.action == ExecutionAction.ENTER_LONG:
                result = await self.signal_handler.handle_entry_long(
                    signal, context, function_name
                )
            elif signal.action == ExecutionAction.ENTER_SHORT:
                result = await self.signal_handler.handle_entry_short(
                    signal, context, function_name
                )
            elif signal.action == ExecutionAction.EXIT:
                result = await self.signal_handler.handle_exit(
                    signal, context, function_name
                )
            elif signal.action == ExecutionAction.MODIFY_STOP:
                result = await self.signal_handler.handle_modify_stop(
                    signal, context, function_name
                )
            else:
                # No action needed
