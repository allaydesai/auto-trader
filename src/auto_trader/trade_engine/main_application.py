...
class TradingApplication:
    ...
    def __init__(self, config: Optional[ApplicationConfig] = None):
        ...
        logger.info(
            f"TradingApplication initialized with config: simulation_mode={self.config.simulation_mode}"
        )
    ...
    async def _initialize_market_data(self) -> None:
        ...
        if not self.ibkr_client or not self.ibkr_client.is_connected():
            logger.warning(
                "IBKR client not connected, skipping market data initialization"
            )
            return
    ...
    async def _subscribe_to_market_data(self) -> None:
        ...
        try:
            ...
            subscription_results = await self.market_data_manager.subscribe_symbols(
                symbols=symbols, bar_sizes=list(timeframes)
            )

            # Log subscription results
            successful_subs = sum(
                1 for result in subscription_results.values() if result
            )
            ...
            if self.trade_orchestrator:
                ...
                if "processors" in orchestrator_stats:
                    ...
                    if "signal_processor_stats" in processors:
                        ...
                        self.app_stats["signals_processed"] = signal_stats.get(
                            "total_processed", 0
                        )
