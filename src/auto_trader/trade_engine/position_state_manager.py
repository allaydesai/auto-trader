...
class PositionStateManager:
    ...
    async def create_position_from_fill(
        ...
        position = PositionEntry(
            ...
            metadata={
                ...
                if hasattr(trade_plan.risk_category, "value")
                else trade_plan.risk_category,
        ...
        logger.info(
            ...
            if hasattr(order_result.side, "value")
            else order_result.side,
        )
        ...
        try:
            ...
            if self.state_file_path.exists():
                backup_path = Path(
                    ...
                    + f".bak.{datetime.now(UTC).strftime('%Y%m%d_%H%M%S')}"
                )
    ...
    def _cleanup_old_backups(self) -> None:
        ...
        try:
            backup_files = list(
                self.state_file_path.parent.glob(f"{self.state_file_path.name}.bak.*")
            )
