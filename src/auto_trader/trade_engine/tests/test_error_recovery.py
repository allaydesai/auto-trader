...
async def error_recovery_system():
    ...
    yield {"registry": registry, "function": function, "detector": detector}
...
class TestErrorRecovery:
    ...
    async def test_execution_logger_disk_space_error(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            logger = ExecutionLogger(
                log_directory=Path(temp_dir), max_entries_per_file=10, max_log_files=3
            )
    ...
    async def test_bar_close_detector_scheduler_failure(self, error_recovery_system):
        ...
        detector.scheduler.add_job = Mock(
            side_effect=Exception("Job scheduling failed")
        )
    ...
    async def test_execution_function_exception_handling(
        ...
        assert (
            "error" in signal.reasoning.lower()
            or "insufficient" in signal.reasoning.lower()
        )
    ...
    async def test_graceful_degradation(self, error_recovery_system):
        ...
        error_recovery_system["detector"]
        ...
        def check_system_health():
            ...
            return {name: status for name, status in components.items()}
    ...
    async def test_configuration_reload_on_error(self, error_recovery_system):
        ...
        backup_config = {"threshold_price": 179.00, "min_volume": 10000}
