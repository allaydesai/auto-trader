...
class PersistentOrderManager:
    ...
    async def place_market_order(self, *args, **kwargs):
        ...
        if args and hasattr(args[0], "symbol"):
            ...
            quantity = getattr(
                order_request, "calculated_position_size", None
            ) or getattr(order_request, "quantity", 100)
            trade_plan_id = order_request.trade_plan_id
        else:
            # Fallback to kwargs
            symbol = kwargs.get("symbol", "UNKNOWN")
            side = (
                OrderSide.BUY if kwargs.get("side", "BUY") == "BUY" else OrderSide.SELL
            )
...
async def persistence_setup(state_manager, persistent_order_manager):
    ...
    logger = ExecutionLogger(
        enable_file_logging=True, log_directory=str(state_manager.base_path / "logs")
    )
...
class TestStatePersistenceIntegration:
    ...
    async def test_audit_trail_integrity(self, persistence_setup):
        ...
        for i, (price, description) in enumerate(trade_scenarios):
            ...
            trigger_bar = create_state_test_bar(
                close_price=price, timestamp=datetime.now(UTC) + timedelta(seconds=i)
            )
        ...
        assert len(state_orders) == len(
            audit_trail
        ), "Order count should match between state and audit trail"
    ...
    async def test_recovery_after_corruption(self, persistence_setup):
        ...
        assert (
            new_order_manager.order_counter == 0
        ), "Should start fresh after corruption"
        assert (
            len(new_order_manager.placed_orders) == 0
        ), "Should have empty orders after corruption"
        ...
        assert (
            new_order_manager.order_counter > 0
        ), "Should have placed orders after corruption recovery"
        assert (
            len(new_order_manager.placed_orders) > 0
        ), "Should have orders in new state"
    ...
    async def test_versioned_state_backups(self, persistence_setup):
        ...
        assert (
            len(versions) >= 3
        ), f"Expected multiple backup versions, got {len(versions)}"
        ...
        for backup_file in versions:
            ...
            try:
                ...
                assert (
                    "order_counter" in backup_data
                ), "Backup should contain order_counter"
    ...
    async def test_historical_data_persistence(self, persistence_setup):
        ...
        for i in range(100):  # Large dataset
            ...
            bar = create_state_test_bar(
                close_price=179.0 + (i % 20) * 0.05, timestamp=bar_time
            )
        ...
        historical_data = {
            "AAPL": {"1min": [bar.model_dump() for bar in historical_bars]}
        }
        ...
        assert (
            len(loaded_data["AAPL"]["1min"]) == 100
        ), "Should have 100 historical bars"
    ...
    async def test_log_rotation_and_archival(self, persistence_setup):
        ...
        for day in range(3):  # Simulate 3 days of activity
            for hour in range(24):
                for minute in range(0, 60, 5):  # Every 5 minutes
                    bar_time = datetime.now(UTC) - timedelta(
                        days=3 - day, hours=24 - hour, minutes=60 - minute
                    )
                    ...
                    if minute % 10 == 0:
                        # Triggering bar
                        bar = create_state_test_bar(
                            close_price=180.25, timestamp=bar_time
                        )
                    else:
                        # Non-triggering bar
                        bar = create_state_test_bar(
                            close_price=179.75, timestamp=bar_time
                        )
        ...
        log_dir = (
            Path(setup["logger"].log_directory)
            if hasattr(setup["logger"], "log_directory")
            else state_manager.base_path / "logs"
        )
        ...
        if len(audit_trail) > 0:
            ...
            first_timestamp = datetime.fromisoformat(
                audit_trail[0]["timestamp"].replace("Z", "+00:00")
            )
            last_timestamp = datetime.fromisoformat(
                audit_trail[-1]["timestamp"].replace("Z", "+00:00")
            )
    ...
    async def test_concurrent_state_persistence(self, persistence_setup):
        ...
        async def concurrent_state_operation(operation_id: int, num_operations: int):
            ...
            for i in range(num_operations):
                ...
                trigger_bar = create_state_test_bar(
                    ...
                    timestamp=datetime.now(UTC)
                    + timedelta(microseconds=operation_id * 1000 + i),
                )
        ...
        assert (
            len(exceptions) == 0
        ), f"Concurrent operations should not fail: {exceptions}"
        ...
        assert (
            len(audit_trail) >= expected_records * 0.8
        ...
        if len(audit_trail) > 0:
            ...
            assert len(order_ids) == len(
                unique_order_ids
            ), "Should not have duplicate order IDs in audit trail"
    ...
    async def test_crash_recovery_simulation(self, persistence_setup):
        ...
        for i in range(3):
            ...
            # Trigger order
            trigger_bar = create_state_test_bar(
                close_price=180.25, timestamp=datetime.now(UTC) + timedelta(seconds=i)
            )
        ...
        assert (
            len(setup["order_manager"].placed_orders) > 0
        ), "Should have orders before crash"

        # Capture pre-crash state
        pre_crash_state = await state_manager.load_execution_state()
        await state_manager.load_positions()
        ...
        new_detector.get_timing_stats = Mock(
            return_value={"avg_detection_latency_ms": 30.0}
        )
        ...
        new_market_adapter.add_signal_callback(
            new_order_adapter.handle_execution_signal
        )
        ...
        assert (
            new_order_manager.order_counter >= recovered_count
