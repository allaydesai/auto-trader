...
class FaultyFunction:
    ...
    async def evaluate(
        self, bar_data: BarData, historical_data: List[BarData]
    ) -> ExecutionSignal:
...
class TestErrorRecoveryIntegration:
    ...
    async def test_corrupted_market_data_handling(self, error_recovery_setup):
        ...
        for corruption_type in corruption_types:
            corrupted_bar = CorruptDataGenerator.create_corrupted_bar(
                corruption_type, base_bar
            )
        ...
        error_logs = [
            log
            for log in logs
            if "error" in str(log).lower() or "invalid" in str(log).lower()
        ]
    ...
    async def test_function_evaluation_failures(self, error_recovery_setup):
        ...
        for mode in failure_modes:
            ...
            setup["registry"].get_functions_by_timeframe = Mock(
                side_effect=mock_get_functions
            )
        ...
        failure_logs = [
            log
            for log in logs
            if "fail" in str(log).lower() or "error" in str(log).lower()
        ]
    ...
    async def test_order_execution_failures_and_retry(self, error_recovery_setup):
        ...
        for attempt in range(total_attempts):
            trigger_bar = create_normal_bar(
                close_price=180.25,
                timestamp=datetime.now(UTC) + timedelta(seconds=attempt),
            )
            await setup["market_adapter"].on_market_data_update(trigger_bar)
        ...
        assert (
            successful_orders > 0
        ), "Some orders should succeed despite network issues"
        ...
        [
            log
            for log in logs
            if "retry" in str(log).lower() or "attempt" in str(log).lower()
        ]
    ...
    async def test_partial_system_failure_isolation(self, error_recovery_setup):
        ...
        failing_logs = [
            log
            for log in logs
            if "failing_function" in str(log) and "error" in str(log).lower()
        ]
    ...
    async def test_memory_pressure_handling(self, error_recovery_setup):
        ...
        ), f"Processing took {processing_time}s, should be under 10s"
    ...
    async def test_cascading_failure_prevention(self, error_recovery_setup):
        ...
        assert (
            successful_processing > 0
        ), "Some processing should succeed despite cascading failures"
        ...
        error_logs = [
            log
            for log in logs
            if "error" in str(log).lower() or "fail" in str(log).lower()
        ]
        success_logs = [
            log
            for log in logs
            if "success" in str(log).lower() or "completed" in str(log).lower()
        ]

        # Should have both error and success logs (showing isolation)
        assert len(error_logs) > 0, "Failures should be logged"
        assert (
            len(success_logs) > 0 or successful_processing > 0
        ), "Successes should also occur"
