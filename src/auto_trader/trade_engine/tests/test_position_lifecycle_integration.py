...
class MockMarketDataProvider:
    ...
    def set_current_price(self, symbol: str, price: Decimal):
        ...
        self.price_history[symbol].append(
            {"price": price, "timestamp": datetime.now(UTC)}
        )
...
class TestPositionLifecycleIntegration:
    ...
    async def test_complete_position_lifecycle(
        ...
        position_id = await position_state_manager.create_position_from_fill(
            trade_plan=sample_trade_plan, order_result=entry_order_result
        )
        ...
        await position_state_manager.close_position(
            position_id=position_id, close_reason="exit_order_fill"
        )
    ...
    async def test_multiple_positions_same_symbol(
        ...
        position_id_1 = await position_state_manager.create_position_from_fill(
            trade_plan=sample_trade_plan, order_result=order_result_1
        )
        ...
        position_id_2 = await position_state_manager.create_position_from_fill(
            trade_plan=trade_plan_2, order_result=order_result_2
        )
        ...
        plan_position_1 = position_state_manager.get_position_by_plan_id(
            "POSITION_TEST_001"
        )
        plan_position_2 = position_state_manager.get_position_by_plan_id(
            "POSITION_TEST_002"
        )
    ...
    @pytest.mark.skip(
        reason="Uses deprecated calculate_unrealized_pnl method - pending future refactor"
    )
    ...
    @pytest.mark.skip(
        reason="Uses deprecated close_position_from_order method - pending future refactor"
    )
    async def test_partial_position_closure(
        self, position_state_manager, sample_trade_plan, entry_order_result
    ):
        ...
        position_id = await position_state_manager.create_position_from_fill(
            trade_plan=sample_trade_plan, order_result=large_entry
        )
        ...
        await position_state_manager.close_position_from_order(
            position_id=position_id, order_result=partial_exit
        )
        ...
        await position_state_manager.close_position_from_order(
            position_id=position_id, order_result=remaining_exit
        )
    ...
    @pytest.mark.skip(
        reason="Uses deprecated API - close_position with exit_order_result parameter"
    )
    async def test_position_summary_generation(
        ...
        await position_state_manager.close_position(
            position_id=position_id, exit_order_result=exit_order_result
        )
    ...
    async def test_position_cleanup_and_archival(
        ...
        await position_state_manager.close_position_from_order(
            position_id=position_id, order_result=exit_order_result
        )
    ...
    async def test_position_error_handling(
        self, position_state_manager, sample_trade_plan
    ):
...
class TestPositionStateManagerPerformance:
    ...
    async def test_concurrent_position_operations(
        self, position_state_manager, temp_state_dir
    ):
        ...
        for i in range(num_positions):
            ...
            task = position_state_manager.create_position_from_fill(
                trade_plan=trade_plan, order_result=order_result
            )
            tasks.append(task)
        ...
        assert (
            creation_time < 5.0
        ), f"Position creation took {creation_time:.2f}s, too slow"

        # Verify file system state
        position_files = list(temp_state_dir.glob("position_*.json"))
        assert len(position_files) == num_positions

    async def test_large_position_set_management(self, position_state_manager):
        ...
        assert (
            retrieval_time < 2.0
        ), f"Position retrieval took {retrieval_time:.2f}s, too slow"
        ...
        assert (
            symbol_retrieval_time < 0.5
        ), f"Symbol retrieval took {symbol_retrieval_time:.2f}s, too slow"
