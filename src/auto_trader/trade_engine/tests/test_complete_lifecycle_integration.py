...
from auto_trader.models.execution import ExecutionFunctionConfig, ExecutionSignal
from auto_trader.models.enums import Timeframe, ExecutionAction, OrderSide, OrderStatus
...
def mock_trade_plan_loader(sample_trade_plan):
    ...
    loader.load_all_plans = AsyncMock(return_value={
        sample_trade_plan.plan_id: sample_trade_plan
    })
...
def mock_risk_manager():
    ...
    risk_manager.position_sizer.calculate_position_size = Mock(return_value=Mock(
        position_size=80,
        dollar_risk=Decimal("200.00"),
...
async def orchestrator(
    ...
    order_risk_validator.validate_order_request = AsyncMock(return_value=Mock(
        is_valid=True,
        errors=[]
    ))
...
def create_bar(
    ...
    return BarData(
        ...
        volume=100000,
        bar_size=timeframe,
        timeframe=Timeframe(timeframe) if hasattr(Timeframe, timeframe.upper().replace("MIN", "_MIN")) else Timeframe.FIFTEEN_MIN
    )
...
class TestCompleteTradeLifecycle:
    ...
    async def test_complete_lifecycle_entry_to_exit(
        ...
        print(f"Entry threshold: {sample_trade_plan.entry_function.parameters['threshold']}")
        ...
        print(f"Entry threshold: 180.50")
        print(f"Expected: NO entry signal")
        ...
        print(f"Entry threshold: 180.50")
        print(f"Expected: ENTRY signal triggered")
        ...
        assert sample_trade_plan.plan_id in orchestrator.position_plans, \
            "Plan should be in position_plans after entry"
        
        assert sample_trade_plan.plan_id not in orchestrator.active_plans, \
            "Plan should be removed from active_plans after entry"
        
        assert len(mock_order_execution_manager.orders_placed) >= 1, \
            "At least one order should have been placed"
        
        print(f"✓ Status changed to: {sample_trade_plan.status}")
        print(f"✓ Plan moved to position_plans: {sample_trade_plan.plan_id in orchestrator.position_plans}")
        print(f"✓ Orders placed: {len(mock_order_execution_manager.orders_placed)}")
        print(f"✓ Active plans: {list(orchestrator.active_plans.keys())}")
        print(f"✓ Position plans: {list(orchestrator.position_plans.keys())}")
        ...
        print(f"Stop loss threshold: 178.00")
        print(f"Expected: NO exit signal (still above stop)")
        ...
        print(f"Stop loss threshold: 178.00")
        print(f"Expected: EXIT signal triggered")
        print(f"\nCurrent state:")
        print(f"  - Plan status: {sample_trade_plan.status}")
        print(f"  - In active_plans: {sample_trade_plan.plan_id in orchestrator.active_plans}")
        print(f"  - In position_plans: {sample_trade_plan.plan_id in orchestrator.position_plans}")
        ...
        assert sample_trade_plan.plan_id not in orchestrator.position_plans, \
            f"❌ Plan should be removed from position_plans after exit"
        
        assert orders_after_exit > orders_before_exit, \
            f"❌ Exit order should have been placed"
    ...
    async def test_timeframe_filtering(
        ...
        if sample_trade_plan.status == TradePlanStatus.POSITION_OPEN:
            ...
            print(f"❌ GAP #3: 15min bar did NOT trigger (status: {sample_trade_plan.status})")
    ...
    async def test_multiple_plans_same_symbol(
        ...
        print("="*70)
        print(f"Plan 1 entry: 180.50")
        print(f"Plan 2 entry: 182.00")
        ...
        assert plan2.status == TradePlanStatus.AWAITING_ENTRY, \
