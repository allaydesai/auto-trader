...
class MockMultiTimeframeDetector:
    ...
    def _check_bar_close(self, symbol: str, timeframe: Timeframe, bar: BarData):
        ...
        if minute <= 5 or minute % 15 == 0 or minute == 59:
            print(
                f"  Checking {timeframe.value}: minute={minute}, should_close={should_close}"
            )
    ...
    def _create_timeframe_bar(
        ...
        if timeframe == Timeframe.ONE_MIN:
            ...
            recent_bars = (
                one_min_bars[-minutes_needed:]
                if len(one_min_bars) >= minutes_needed
                else one_min_bars
            )
    ...
    def _get_next_close_time(
        self, timeframe: Timeframe, current_time: datetime
    ) -> datetime:
...
def create_timeframe_bar_sequence(
    ...
    for i in range(duration_minutes):
        ...
        price_change = Decimal(str(price_trend * i / duration_minutes)).quantize(
            Decimal("0.0001")
        )
...
class TestMultiTimeframeIntegration:
    ...
    async def test_simultaneous_timeframe_monitoring(self, multi_timeframe_setup):
        ...
        start_time = (datetime.now(UTC) - timedelta(hours=2)).replace(
        ...
        for i, bar in enumerate(bars):
            ...
            if (
                i < 10 or minute % 5 == 0 or minute % 15 == 0 or minute == 59
            ):  # Log some key moments
                print(f"Bar {i}: {bar.timestamp} (minute={minute}, hour={hour})")
                print(f"  - 5min would trigger: {minute % 5 == 4 or minute % 5 == 0}")
                print(
                    f"  - 15min would trigger: {minute % 15 == 14 or minute % 15 == 0}"
                )
        ...
        assert (
            len(timeframe_evaluations) >= 2
        ), f"Expected multiple timeframes, got: {timeframe_evaluations}"
    ...
    async def test_timeframe_synchronization(self, multi_timeframe_setup):
        ...
        assert (
            len(fifteen_min_logs) >= 1
        ), "15-minute timeframe should have been evaluated"
    ...
    async def test_cross_timeframe_signal_correlation(self, multi_timeframe_setup):
        ...
        assert (
            len(signal_counts) >= 2
        ), f"Expected correlated signals, got: {signal_counts}"
    ...
    async def test_multi_timeframe_performance(self, multi_timeframe_setup):
        ...
        # Use past timestamp to avoid future data validation errors
        past_start_time = (datetime.now(UTC) - timedelta(hours=2)).replace(
            minute=0, second=0, microsecond=0
        )
        bars = create_timeframe_bar_sequence(
            symbol="AAPL",
        ...
        bars_per_second = (
            len(bars) / processing_time if processing_time > 0 else float("inf")
        )
        assert (
            bars_per_second > 50
        ), f"Processing rate too slow: {bars_per_second} bars/sec"
        ...
        if "avg_evaluation_time_ms" in metrics:
            assert (
                metrics["avg_evaluation_time_ms"] < 100.0
            ), "Function evaluation too slow"
    ...
    async def test_multi_symbol_multi_timeframe(self, multi_timeframe_setup):
        ...
        ), f"Expected multiple symbols in logs, got: {symbols_in_logs}"
        assert (
            len(timeframes_in_logs) >= 2
        ), f"Expected multiple timeframes in logs, got: {timeframes_in_logs}"
    ...
    async def test_timeframe_cleanup_on_stop(self, multi_timeframe_setup):
        ...
        assert (
            "AAPL" not in monitored_after_full
            or len(monitored_after_full.get("AAPL", [])) == 0
        )
