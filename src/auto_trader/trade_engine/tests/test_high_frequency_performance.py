...
async def performance_detector():
    ...
    detector = BarCloseDetector(accuracy_ms=100, schedule_advance_ms=50)
...
class TestHighFrequencyPerformance:
    ...
    async def test_stress_test_all_components(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            logger = ExecutionLogger(
                log_directory=Path(temp_dir), max_entries_per_file=100, max_log_files=5
            )
            ...
            print(
                f"Overall throughput: {total_evaluations / total_time:.1f} ops/second"
            )
