...
def create_test_trade_plan(
    plan_id: str,
    symbol: str = "AAPL",
    status: TradePlanStatus = TradePlanStatus.AWAITING_ENTRY,
):
...
class TestTradeLifecycleManager:
    ...
    def test_create_duplicate_state_raises_error(
        self, lifecycle_manager, sample_trade_plan
    ):
    ...
    def test_get_lifecycle_state(self, lifecycle_manager, sample_trade_plan):
        ...
        retrieved_state = lifecycle_manager.get_lifecycle_state(
...
class TestStateTransitions:
    ...
    @pytest.mark.asyncio
    async def test_transition_to_completed_invalid(
        self, lifecycle_manager, sample_trade_plan
    ):
    ...
    async def test_transition_to_completed_no_state(self, lifecycle_manager):
        ...
        plan = create_test_trade_plan(
            "TEST_NO_STATE", status=TradePlanStatus.POSITION_OPEN
        )
        exit_result = Mock()
    ...
    async def test_transition_to_cancelled_invalid(self, lifecycle_manager):
        ...
        plan = create_test_trade_plan(
            "TEST_COMPLETED", status=TradePlanStatus.COMPLETED
        )
...
class TestQueryMethods:
    ...
    def test_get_plans_by_status(self, lifecycle_manager):
        ...
        plan3 = create_test_trade_plan(
            "PLAN_3", "GOOGL", TradePlanStatus.AWAITING_ENTRY
        )
        ...
        awaiting_plans = lifecycle_manager.get_plans_by_status(
            TradePlanStatus.AWAITING_ENTRY
        )
        position_plans = lifecycle_manager.get_plans_by_status(
            TradePlanStatus.POSITION_OPEN
        )
        completed_plans = lifecycle_manager.get_plans_by_status(
...
class TestStateValidation:
    ...
    def test_validate_state_consistency_invalid(self, lifecycle_manager):
        ...
        assert any(
            "POSITION_OPEN but has no position quantity" in error for error in errors
        )
    ...
    def test_validation_caching(self, lifecycle_manager):
        ...
        assert (
            len(lifecycle_manager.validator._validation_cache) > cache_size_after_first
        )
