...
class TestEdgeCaseDetector:
    ...
    def test_limit_move_detection(self, limit_up_scenario):
        ...
        assert any(
            pct in result.description for pct in ["12", "13", "14", "15", "16"]
        )  # ~12-13% move
    ...
    def test_volume_spike_detection(self, volatile_market_bars):
        ...
        ), f"No bar found with volume >= {threshold_volume}, max volume was {max_volume}"

        # Use bars with lower volume as historical context
        min(volumes)
        median_volume = volumes_sorted[len(volumes_sorted) // 2]
        historical_bars = [
            bar for bar in volatile_market_bars if bar.volume <= median_volume
        ][:20]
...
class TestExecutionFunctionsWithEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_close_above_skips_invalid_data(
        self, close_above_function, invalid_data_bars, trending_up_bars
    ):
        """Test close above function skips evaluation for invalid data."""
        current_bar = invalid_data_bars[
            1
        ]  # This is actually valid data due to pydantic validation
    ...
    @pytest.mark.asyncio
    async def test_close_above_adjusts_confidence_for_gaps(
        self, close_above_function, gap_up_scenario, trending_up_bars
    ):
        ...
        historical_bars = base_historical + [
            gap_bars[0]
        ]  # Add the bar before gap (20 total)
    ...
    @pytest.mark.asyncio
    async def test_close_below_handles_gap_down(
        self,
        close_below_function,
        gap_down_scenario,
    ...
    @pytest.mark.asyncio
    async def test_functions_handle_low_volume(
        self, close_above_function, low_volume_bars, trending_up_bars
    ):
        ...
        if signal.action == ExecutionAction.ENTER_LONG:
            ...
            assert (
                "volume" in signal.reasoning.lower()
                or "edge case" in signal.reasoning.lower()
            )

    @pytest.mark.asyncio
    async def test_distance_validation_parameters(
        self, close_above_function, trending_up_bars
    ):
        ...
        historical_bars = (
            bars if len(bars) >= 20 else bars + trending_up_bars[: 20 - len(bars)]
        )
...
class TestMarketScenarios:
    ...
    @pytest.mark.asyncio
    async def test_trending_market_scenarios(
        self, trending_up_bars, trending_down_bars
    ):
        ...
        historical_bars = (
            ...
            else trending_up_bars[:19]
        )
        if len(historical_bars) < 20:
            historical_bars = (
                historical_bars + trending_up_bars[: 20 - len(historical_bars)]
            )
    ...
    async def test_ranging_market_scenarios(self, ranging_market_bars):
        ...
        for i in range(5, len(ranging_market_bars)):
            ...
            if len(historical_bars) < 20:
                ...
                base_bar = (
                    historical_bars[0] if historical_bars else ranging_market_bars[0]
                )
