...
def create_market_scenario(
    ...
    for i in range(num_bars):
        ...
        if price_pattern == "uptrend":
            trend = Decimal(
                str(2.5 * i / num_bars)
            )  # Gradual uptrend (ensures final > 180.0)
        ...
        if price_pattern == "gap_up" and i == num_bars - 1:
            ...
            close_variation = (
                Decimal(str(volatility)) * Decimal("0.2") * Decimal(str(0.5 - i % 2))
            )
            close_price = (
                open_price + close_variation.quantize(Decimal("0.0001"))
            ).quantize(Decimal("0.0001"))
            high_price = (
                open_price
                + (Decimal(str(volatility)) * Decimal("0.5")).quantize(
                    Decimal("0.0001")
                )
            ).quantize(Decimal("0.0001"))
            low_price = (
...
class TestEnhancedIntegration:
    ...
    async def test_multi_symbol_execution_workflow(self, complete_integration_setup):
        ...
        for symbol_config in symbols_configs:
            ...
            await setup["market_adapter"].start_monitoring(
                symbol_config["symbol"], Timeframe.ONE_MIN
            )
    ...
    async def test_complex_market_scenarios(self, complete_integration_setup):
        ...
        for pattern, should_trigger, description in scenarios:
            ...
            if should_trigger:
                assert (
                    setup["order_manager"].place_market_order.call_count >= 1
                ), description
    ...
    async def test_performance_under_load(self, complete_integration_setup):
        ...
        assert (
            processing_time < 5.0
        ), f"Processing took {processing_time}s, should be under 5s"
    ...
    async def test_position_lifecycle_integration(self, complete_integration_setup):
        ...
        ), "Exit order should increase total order count"
    ...
    async def test_data_flow_validation(self, complete_integration_setup):
        ...
        original_update_data = setup[
            "market_adapter"
        ].historical_data_manager.update_data
        ...
        setup[
            "market_adapter"
        ].historical_data_manager.update_data = track_historical_update
        ...
        try:
            ...
            if setup["order_manager"].place_market_order.call_count > 0:
                flow_tracking["order_placed"] = setup[
                    "order_manager"
                ].place_market_order.call_count

        finally:
            # Restore original methods using composition pattern components
            setup[
                "market_adapter"
            ].historical_data_manager.update_data = original_update_data
        ...
        assert (
            flow_tracking["signal_generated"] >= 1 or flow_tracking["order_placed"] == 0
        )
    ...
    async def test_audit_trail_completeness(self, complete_integration_setup):
        ...
        trigger_bar = create_market_scenario(
            symbol="AAPL", base_price=180.5, num_bars=1, price_pattern="sideways"
        )[0]
        ...
        [log for log in logs if log.signal.should_execute]
        ...
        for metric in required_metrics:
            assert (
                metric in metrics
            ), f"Required metric {metric} missing from audit trail"
