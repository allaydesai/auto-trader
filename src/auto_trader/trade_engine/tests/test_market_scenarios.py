...
def create_valid_bar(
    ...
    if open_price is None:
        ...
        open_price = close_price + (
            Decimal("0.10") if close_price > Decimal("100") else -Decimal("0.10")
        )
    else:
        open_price = open_price.quantize(Decimal("0.0001"))

    # Ensure proper OHLC relationships
    high_price = (max(open_price, close_price) + Decimal("0.30")).quantize(
        Decimal("0.0001")
    )
    low_price = (min(open_price, close_price) - Decimal("0.20")).quantize(
        Decimal("0.0001")
    )
...
def ensure_sufficient_bars(bars_list, min_count=20):
    ...
    for i in range(min_count - len(bars_list)):
        additional_bars.append(
            create_valid_bar(
                symbol=base_bar.symbol,
                timestamp=base_bar.timestamp,
                close_price=(base_bar.close_price + Decimal(str(i * 0.1))).quantize(
                    Decimal("0.0001")
                ),
...
class TestCloseAboveFunctionScenarios:
    ...
    @pytest.mark.asyncio
    async def test_bear_trend_false_breakout(
        self, close_above_function, trending_down_bars
    ):
    ...
    @pytest.mark.asyncio
    async def test_ranging_market_whipsaw(
        self, close_above_function, ranging_market_bars
    ):
        ...
        for i in range(10, len(ranging_market_bars)):
            ...
            close_price = (
                Decimal("102.10")
                if original_bar.close_price > Decimal("102.0")
                else original_bar.close_price
            )
...
class TestCloseBelowFunctionScenarios:
    ...
    @pytest.mark.asyncio
    async def test_stop_loss_trigger_in_downtrend(
        self, close_below_exit_function, trending_down_bars, sample_position_long
    ):
    ...
    @pytest.mark.asyncio
    async def test_short_entry_breakdown(
        self, close_below_short_function, trending_down_bars
    ):
    ...
    @pytest.mark.asyncio
    async def test_no_position_for_exit(
        self, close_below_exit_function, trending_down_bars
    ):
    ...
    @pytest.mark.asyncio
    async def test_position_exists_for_short_entry(
        self, close_below_short_function, trending_down_bars, sample_position_long
    ):
...
class TestTrailingStopFunctionScenarios:
    ...
    @pytest.mark.asyncio
    async def test_trailing_in_uptrend(
        self, trailing_stop_function, trending_up_bars, sample_position_long
    ):
        ...
        for i in range(10, len(trending_up_bars)):
            ...
            if signal.action == ExecutionAction.MODIFY_STOP:
                ...
                new_stop = Decimal(
                    str(signal.metadata.get("new_stop_level", position.stop_loss))
                )
        ...
        stop_levels = [
            Decimal(str(s.metadata["new_stop_level"])) for s in modify_signals
        ]
        assert stop_levels[-1] > stop_levels[0]  # Stop moved up

    @pytest.mark.asyncio
    async def test_trailing_stop_hit(
        self, trailing_stop_function, volatile_market_bars, sample_position_long
    ):
    ...
    @pytest.mark.asyncio
    async def test_no_position_resets_tracking(
        self, trailing_stop_function, trending_up_bars
    ):
    ...
    @pytest.mark.asyncio
    async def test_volatility_adjusted_trailing(
        self, volatile_market_bars, sample_position_long
    ):
                ...
                for i in range(additional_count):
                    additional_bars.append(
                        BarData(
                            ...
                            close_price=base_bar.close_price,
                            volume=base_bar.volume,
                            bar_size=bar_size_map[
                                timeframe
                            ],  # Use appropriate bar_size
                        )
                    )
                historical_bars = additional_bars + historical_bars
