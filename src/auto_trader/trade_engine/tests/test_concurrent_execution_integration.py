...
from auto_trader.models.execution import ExecutionFunctionConfig, BarCloseEvent
...
class ThreadSafeOrderManager:
    ...
    def get_concurrent_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_processing_time": sum(self.processing_delays.values())
            / len(self.processing_delays)
...
class ConcurrentBarDetector:
    ...
    async def _process_bar_close(self, symbol: str, timeframe: Timeframe, bar: BarData):
        ...
        self.max_concurrent_processing = max(
            self.max_concurrent_processing, self.processing_count
        )
    ...
    def get_timing_stats(self) -> Dict:
        return {
            "avg_detection_latency_ms": 25.0,
            "total_detections": len(self.bar_close_events),
        }

    def get_monitored(self) -> Dict:
        return {
            symbol: list(timeframes)
            for symbol, timeframes in self.monitored_symbols.items()
        }
...
def create_concurrent_bar_data(
    symbols: List[str], num_bars: int, base_prices: Dict[str, float]
    ...
    for symbol in symbols:
        ...
        for i in range(num_bars):
            ...
            price_variation = Decimal(str((i % 10 - 5) * 0.1000)).quantize(
                Decimal("0.0001")
            )
            ...
            bar = BarData(
                symbol=symbol,
                timestamp=bar_time,
                open_price=(close_price - Decimal("0.0500")).quantize(
                    Decimal("0.0001")
                ),
                high_price=(close_price + Decimal("0.1000")).quantize(
                    Decimal("0.0001")
                ),
...
class TestConcurrentExecutionIntegration:
    ...
    async def test_concurrent_symbol_processing(self, concurrent_execution_setup):
        ...
        assert (
            concurrent_stats["max_concurrent_processing"] > 1
        ), "Should have processed multiple symbols concurrently"
        ...
        assert len(processed_symbols) == len(
            symbols
        ), f"Expected all {len(symbols)} symbols, got {len(processed_symbols)}"
    ...
    async def test_race_condition_prevention(self, concurrent_execution_setup):
        ...
        assert (
            len(successes) >= concurrent_triggers * 0.8
        ), f"Expected mostly successes, got {len(successes)}/{concurrent_triggers}"

        # Verify order manager handled concurrency correctly
        concurrent_stats = setup["order_manager"].get_concurrent_stats()
        assert (
            concurrent_stats["max_concurrent"] >= 2
        ), "Should have had concurrent order processing"
    ...
    async def test_high_frequency_concurrent_updates(self, concurrent_execution_setup):
        ...
        async def generate_high_frequency_stream(
            symbol: str, duration_seconds: int, bars_per_second: int
        ):
            """Generate high-frequency bar stream."""
            base_price = (
                180.0 if symbol == "AAPL" else 2800.0 if symbol == "GOOGL" else 420.0
            )
            ...
            for i in range(total_bars):
                ...
                base_decimal = Decimal(str(base_price))
                price_variation = Decimal(str((i % 20 - 10) * 0.0100)).quantize(
                    Decimal("0.0001")
                )
                close_price = (base_decimal + price_variation).quantize(
                    Decimal("0.0001")
                )
                ...
                bar = BarData(
                    symbol=symbol,
                    timestamp=bar_time,
                    open_price=(close_price - Decimal("0.0200")).quantize(
                        Decimal("0.0001")
                    ),
                    high_price=(close_price + Decimal("0.0500")).quantize(
                        Decimal("0.0001")
                    ),
                    low_price=(close_price - Decimal("0.0500")).quantize(
                        Decimal("0.0001")
                    ),
        ...
        for symbol in symbols:
            task = asyncio.create_task(
                generate_high_frequency_stream(
                    symbol, duration_seconds=5, bars_per_second=10
                )
            )
        ...
        ), f"High-frequency processing took {processing_time}s, should be under 10s"

        # Check concurrent processing statistics
        detector_stats = setup["detector"].get_concurrent_stats()
        setup["order_manager"].get_concurrent_stats()

        assert (
            detector_stats["max_concurrent_processing"] >= 2
        ), "Should have concurrent bar processing"
        ...
        if "total_evaluations" in metrics:
            ...
            ), "Should have processed many evaluations"
    ...
    async def test_resource_contention_handling(self, concurrent_execution_setup):
        ...
        async def create_resource_contention(task_id: int):
            ...
            for i in range(25):
                bar = BarData(
                    symbol="AAPL",
                    timestamp=datetime.now(UTC)
                    - timedelta(minutes=25 - i)
                    + timedelta(microseconds=task_id),
                    open_price=Decimal("179.50"),
                    high_price=Decimal("180.00"),
                    low_price=Decimal("179.00"),
            ...
            trigger_bar = BarData(
                ...
                + Decimal(str(task_id)) * Decimal("0.0100"),  # Different prices
                volume=2000000,
                bar_size="1min",
            )
        ...
        assert (
            len(successful_tasks) >= 3
        ), f"Expected most tasks to succeed, got {len(successful_tasks)}/5"
    ...
    async def test_thread_safety_validation(self, concurrent_execution_setup):
        ...
        async def concurrent_state_updater(updater_id: int, num_updates: int):
            ...
            for i in range(num_updates):
                ...
                bar = BarData(
                    symbol="AAPL",
                    timestamp=datetime.now(UTC)
                    + timedelta(microseconds=updater_id * 1000 + i),
                    open_price=Decimal("179.90"),
                    high_price=Decimal("180.50"),
                    low_price=Decimal("179.50"),
                    close_price=Decimal("179.5000")
                    + Decimal(str(i)) * Decimal("0.0100"),
                    volume=1000000,
                    bar_size="1min",
                )
        ...
        if len(order_history) > 1:
            ...
            if len(unique_timestamps) < len(timestamps):
                ...
                assert (
                    duplicate_count < 3
                ), f"Too many potential duplicate orders: {duplicate_count}"
    ...
    async def test_concurrent_function_registration(self, concurrent_execution_setup):
        ...
        async def concurrent_function_registration(
            registrar_id: int, num_functions: int
        ):
        ...
        [r for r in registration_results if not r["success"]]

        success_rate = len(successful_registrations) / len(registration_results)
        assert (
            success_rate > 0.9
        ), f"Registration success rate too low: {success_rate:.2%}"
        ...
        assert len(function_names) == len(
            unique_names
        ), "Function name conflicts detected"
        ...
        assert (
            len(evaluation_logs) >= 5
        ), f"Expected multiple function evaluations, got {len(evaluation_logs)}"
    ...
    async def test_deadlock_prevention(self, concurrent_execution_setup):
        ...
        async def deadlock_prone_operation(operation_id: int):
            ...
            for i in range(20):
                bar = BarData(
                    ...
                    + timedelta(microseconds=operation_id),
                    open_price=Decimal("179.50"),
                    high_price=Decimal("180.00"),
                    low_price=Decimal("179.00"),
                    close_price=Decimal("179.75"),
                ...
                await setup["registry"].get_functions_by_timeframe(
        ...
        timeout_logs = [
            log
            for log in logs
            if "timeout" in str(log).lower() or "deadlock" in str(log).lower()
        ]

        # Should not have timeout/deadlock indicators
        assert (
            len(timeout_logs) == 0
        ), f"Found potential deadlock indicators: {len(timeout_logs)}"
