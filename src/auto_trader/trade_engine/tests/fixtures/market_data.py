...
def ranging_market_bars() -> List[BarData]:
    ...
    for i in range(20):
        ...
        close_price = (open_price + Decimal(str(wave * 0.3))).quantize(
            Decimal("0.0001")
        )  # Some momentum
        ...
        bars.append(
            BarData(
                ...
                close_price=close_price,
                volume=1000000
                + int(abs(wave) * 100000),  # Volume varies with price movement
                bar_size="1min",
            )
        )
...
def volatile_market_bars() -> List[BarData]:
    ...
    for i in range(20):
        ...
        close_price = (open_price + (movement * Decimal("0.5"))).quantize(
            Decimal("0.0001")
        )

        # Ensure proper OHLC relationships
        high_price = (max(open_price, close_price) + Decimal("1.00")).quantize(
            Decimal("0.0001")
        )
        low_price = (min(open_price, close_price) - Decimal("1.00")).quantize(
            Decimal("0.0001")
        )
        ...
        bars.append(
            BarData(
                ...
                close_price=close_price,
                volume=int(
                    500000 + abs(float(movement)) * 200000
                ),  # High volume on big moves, convert to int
                bar_size="1min",
            )
        )
