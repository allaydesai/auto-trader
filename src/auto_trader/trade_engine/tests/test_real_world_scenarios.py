...
class RealWorldMarketDataGenerator:
    ...
    def create_market_session_data(
        ...
        for i in range(duration_minutes):
            ...
            price_change = cls._calculate_price_change(
                condition, i, duration_minutes, chars["volatility_multiplier"]
            )
            volume_modifier = cls._calculate_volume_modifier(
                condition, i, duration_minutes
            )
            ...
            high_price = (
                ...
                + Decimal(str(volatility * random.uniform(0, 1)))
    ...
    def _calculate_price_change(
        ...
        if condition == MarketCondition.TRENDING_UP:
            ...
            return Decimal(
                str(random.uniform(-base_volatility * 3, base_volatility * 3))
            )

        elif condition == MarketCondition.LOW_VOLATILITY:
            return Decimal(
                str(random.uniform(-base_volatility * 0.3, base_volatility * 0.3))
            )
    ...
    def _calculate_volume_modifier(
        condition: MarketCondition, minute_index: int, total_minutes: int
    ) -> float:
...
class NewsEventSimulator:
    ...
    @staticmethod
    def create_fomc_announcement_scenario(
        symbols: List[str], base_prices: Dict[str, float]
...
class TestRealWorldScenarios:
    ...
    async def test_market_open_volatility_scenario(self, real_world_setup):
        ...
        assert (
            triggered_orders > 0
        ), "Should have triggered executions during market open volatility"

        # Verify execution logging captured market session characteristics
        logs = await setup["logger"].query_logs(limit=100)
        [log for log in logs if "market_open" in str(log).lower()]
    ...
    async def test_earnings_announcement_scenario(self, real_world_setup):
        ...
        for scenario in scenarios:
            ...
            earnings_data = NewsEventSimulator.create_earnings_announcement_scenario(
                symbol="AAPL", base_price=180.0, surprise_type=scenario
            )
            ...
            for bar in earnings_data:
                ...
                if abs(bar.close_price - Decimal("180.0")) > Decimal(
                    "1.0"
                ) or bar.close_price > Decimal("182.0"):
                    bar_close_event = BarCloseEvent(
                        symbol="AAPL",
                        timeframe=Timeframe.ONE_MIN,
                        close_time=bar.timestamp,
            ...
            if scenario == "beat":
                ...
                assert (
                    setup["order_manager"].place_market_order.call_count > 0
                ), "Earnings beat should trigger breakout"
    ...
    async def test_fomc_announcement_multi_symbol_impact(self, real_world_setup):
        ...
        fomc_data = NewsEventSimulator.create_fomc_announcement_scenario(
            symbols, base_prices
        )
        ...
        async def process_symbol_fomc_data(symbol: str, bars: List[BarData]):
            for bar in bars:
                ...
                if abs(bar.close_price - Decimal(str(base_price))) > Decimal(
                    str(move_threshold)
                ):
                    bar_close_event = BarCloseEvent(
                        symbol=symbol,
        ...
        assert (
            len(symbols_in_logs) >= 2
        ), f"Expected multi-symbol impact, got: {symbols_in_logs}"
    ...
    async def test_options_expiration_scenario(self, real_world_setup):
        ...
        setup["order_manager"].place_market_order.call_count
        ...
        [
            log
            for log in logs
            if "volatility" in str(log).lower() or "volume" in str(log).lower()
        ]
        ...
        assert (
            metrics["total_evaluations"] > 50
        ), "Should have many evaluations during volatile expiration"
    ...
    async def test_trading_halt_and_resumption(self, real_world_setup):
        ...
        post_resumption_orders = (
            setup["order_manager"].place_market_order.call_count - pre_halt_orders
        )

        # Verify resumption triggered executions
        assert (
            post_resumption_orders > 0
        ), "Trading resumption should trigger executions"
        ...
        [
            ...
            for log in logs
            if any(
                word in str(log).lower() for word in ["gap", "volatile", "resumption"]
            )
        ]
        ...
        ), "Should have evaluations before and after halt"
    ...
    async def test_month_end_rebalancing_scenario(self, real_world_setup):
        ...
        for symbol in rebalancing_symbols:
            ...
            volume_config = ExecutionFunctionConfig(
                ...
                timeframe=Timeframe.ONE_MIN,
                parameters={
                    "threshold_price": base_prices[symbol] * 1.005
                },  # 0.5% threshold
                enabled=True,
            )
        ...
        active_symbols = len(
            [s for s, count in symbol_evaluations.items() if count > 0]
        )
        assert (
            active_symbols >= 2
        ), f"Expected multi-symbol rebalancing activity, got {active_symbols} symbols"
    ...
    async def test_weekend_gap_monday_scenario(self, real_world_setup):
        ...
        weekend_news_impact = random.choice(
            [0.03, -0.025, 0.015]
        )  # 3% up, 2.5% down, or 1.5% up
        ...
        monday_premarket = RealWorldMarketDataGenerator.create_market_session_data(
            ...
            condition=MarketCondition.GAP_UP
            if weekend_news_impact > 0.02
            else MarketCondition.GAP_DOWN
            if weekend_news_impact < -0.02
        ...
        if abs(weekend_news_impact) > 0.02:  # Significant gap
            ...
            [
                log
                for log in logs
                if "gap" in str(log).lower() or str(monday_open_price) in str(log)
            ]
    ...
    async def test_intraday_session_transitions(self, real_world_setup):
        ...
        for session in sessions:
            ...
            for i, bar in enumerate(session_data):
                if i % 10 == 5:  # Every 10th bar starting from 5th
                    ...
                    trigger_high = max(
                        bar.open_price, trigger_close, Decimal("180.5000")
                    )
                    ...
                    # Trigger bar close event via detector
                    await setup["detector"].trigger_bar_close(
                        "AAPL", Timeframe.ONE_MIN, trigger_bar
                    )
                else:
                    await setup["market_adapter"].on_market_data_update(bar)
            ...
            session_orders = (
                setup["order_manager"].place_market_order.call_count - initial_orders
            )
        ...
        assert (
            total_session_orders > 0
        ), "Should have executions across trading sessions"
        ...
        for log in logs:
            ...
            for session in sessions:
                if session.value in log_str:
                    session_activity[session.value] = (
                        session_activity.get(session.value, 0) + 1
                    )
        ...
        assert (
            active_sessions >= 0
        ), f"Expected multi-session activity, got {active_sessions} sessions"
