...
from auto_trader.models.execution import BarCloseEvent, ExecutionSignal
...
def market_data_adapter(
    mock_bar_close_detector, mock_function_registry, mock_execution_logger
):
...
class TestMarketDataExecutionAdapter:
    ...
    @pytest.mark.asyncio
    async def test_bar_close_with_function_evaluation(
        self, market_data_adapter, sample_bar
    ):
        ...
        mock_function.evaluate = AsyncMock(
            return_value=ExecutionSignal(
    ...
    @pytest.mark.asyncio
    async def test_function_evaluation_with_signal(
        self, market_data_adapter, sample_bar
    ):
