...
def create_test_signal(action: ExecutionAction = ExecutionAction.ENTER_LONG, confidence: float = 0.8) -> ExecutionSignal:
    """Create a test execution signal."""
    return ExecutionSignal(action=action, confidence=confidence, reasoning="Test signal for optimization testing")
...
class TestSignalValidatorOptimized:
    ...
    def test_efficient_signal_cleanup(self, validator):
        ...
        is_duplicate = validator.check_duplicate_signal(signal, plan)
    ...
    def test_signal_cleanup_preserves_order(self, validator):
        ...
        for i in range(5):
            key = f"signal_{i}"
            time_offset = datetime.now(UTC) - timedelta(seconds=400 - i * 50)  # Oldest to newest
        ...
        assert all(time >= datetime.now(UTC) - timedelta(seconds=300) for time in remaining_times[:-1])  # Exclude the just-added signal
