...
class PositionPnLCalculator:
    ...
    def calculate_portfolio_unrealized_pnl(
        positions: List[PositionEntry], current_prices: Dict[str, Decimal]
    ) -> Decimal:
        ...
        for position in positions:
            ...
            if current_price is None:
                logger.warning(
                    f"No current price for {position.symbol}, skipping P&L calculation"
                )
                continue
    ...
    @staticmethod
    def get_position_summary_by_symbol(
        ...
        for symbol_stats in symbol_summary.values():
            symbol_stats["total_dollar_value"] = float(
                symbol_stats["total_dollar_value"]
            )
    ...
    def get_portfolio_summary(positions: List[PositionEntry]) -> Dict[str, Any]:
        ...
        total_realized_pnl = PositionPnLCalculator.calculate_portfolio_realized_pnl(
            positions
        )
        ...
        # Group by symbol
        symbol_breakdown = PositionPnLCalculator.get_position_summary_by_symbol(
            positions
        )
        ...
        avg_position_size = (
            total_dollar_value / len(positions) if positions else Decimal("0")
        )

        # Calculate win/loss statistics for closed positions
        winning_positions = [
            pos for pos in closed_positions if pos.calculate_realized_pnl() > 0
        ]
        losing_positions = [
            pos for pos in closed_positions if pos.calculate_realized_pnl() < 0
        ]
