...
from auto_trader.models.enums import OrderSide, ExecutionAction
...
class ExitOrderBuilder:
    ...
    def create_exit_order_request(
        ...
        order_metadata = {
            ...
            "function_name": metadata.get("function_name", "unknown") if metadata else "unknown",
    ...
    def validate_exit_order_request(
        ...
        try:
            ...
            if order_quantity > abs(position.remaining_quantity):
                logger.error(f"Exit quantity {order_quantity} exceeds position {position.remaining_quantity}")
                return False
            ...
            logger.debug(f"Exit order request validation passed for position {position.position_id}")
            return True
