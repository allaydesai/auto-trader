...
class TradeStatisticsCalculator:
    ...
    def get_orchestrator_status_summary(
        ...
        if signal_processor:
            try:
                processors["signal_processor_stats"] = signal_processor.get_processing_statistics()
        ...
        if exit_processor:
            try:
                processors["exit_processor_stats"] = exit_processor.get_processing_statistics()
            except Exception as e:
        ...
        if lifecycle_manager:
            try:
                processors["lifecycle_manager_stats"] = lifecycle_manager.get_lifecycle_statistics()
            except Exception as e:
        ...
        try:
            processors["position_manager_summary"] = position_manager.get_position_summary()
        except Exception as e:
    ...
    def calculate_portfolio_risk_metrics(
        ...
        if open_positions:
            ...
            concentration = float(largest_position / total_exposure) if total_exposure > 0 else 0
        else:
            concentration = 0
        ...
        return {
            ...
            "average_position_size": float(total_exposure / len(open_positions)) if open_positions else 0,
            "largest_position": float(max(pos.dollar_value for pos in open_positions)) if open_positions else 0,
        }
