...
class SignalProcessor:
    ...
    async def process_entry_signal(
        ...
        try:
            ...
            if not signal:
                ...
                logger.warning(
                    "No signal provided and function evaluation not implemented"
                )
                return SignalProcessingResult(
                    success=False,
            ...
            if signal.action == ExecutionAction.ENTER_LONG:
                return await self._process_long_entry(
                    trade_plan, context, signal, function_name
                )
    ...
    async def _process_long_entry(
        ...
        try:
            ...
            order_request = (
                self.execution_adapter.order_request_builder.create_entry_order(
                    symbol=trade_plan.symbol,
                    side=OrderSide.BUY,
                    signal=signal,
                    context=context,
            ...
            # Execute order
            order_result = (
                await self.execution_adapter.order_execution_manager.place_market_order(
                    order_request
                )
            )
    ...
    def _create_signal_data(
        self, signal: ExecutionSignal, function_name: str
    ) -> Dict[str, Any]:
