...
class TrailingStopFunction(ExecutionFunctionBase, ValidationMixin):
    ...
    def validate_parameters(self, params: Dict[str, Any]) -> bool:
        ...
        if "trail_amount" in params:
            ...
            logger.warning(
                "Both trail_percentage and trail_amount specified, using trail_percentage"
            )
    ...
    async def evaluate(self, context: ExecutionContext) -> ExecutionSignal:
        ...
        if trail_amount:
            new_stop_level = self._calculate_stop_level_fixed_amount(
                position, Decimal(str(trail_amount)), initial_stop
            )
        else:
            new_stop_level = self._calculate_stop_level(
                position, trail_pct, initial_stop
            )
    ...
    def _update_extremes(self, bar: "BarData", position: "PositionState") -> None:
        ...
        if not bar or not position:
            logger.warning(
                f"{self.name}: Invalid bar or position data for extreme tracking"
            )
            return
    ...
    def _adjust_trail_for_volatility(
        self, base_trail_pct: Decimal, context: ExecutionContext
    ) -> Decimal:
        ...
        if atr_pct > Decimal("3"):  # High volatility
            ...
            adjustment = max(
                Decimal("0.8"), Decimal("1.0") - (Decimal("1") - atr_pct) / Decimal("5")
            )
        ...
        return max(
            Decimal("0.001"), min(Decimal("0.10"), adjusted_trail)
        )  # 0.1% to 10%
