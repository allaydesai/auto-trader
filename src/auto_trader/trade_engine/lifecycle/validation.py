...
class LifecycleValidator:
    ...
    def _validate_single_state(
        self, plan_id: str, state: TradeLifecycleState
    ) -> List[str]:
        ...
        if state.has_position and state.entry_price is None:
            errors.append(f"Plan {plan_id} has position but no entry price recorded")
        ...
        if state.position_quantity < 0 and not state.is_short:
            errors.append(f"Plan {plan_id} has negative quantity but is_short is False")

        if state.position_quantity > 0 and not state.is_long:
            errors.append(f"Plan {plan_id} has positive quantity but is_long is False")
