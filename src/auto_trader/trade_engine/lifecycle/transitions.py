...
class StateTransitionManager:
    ...
    async def transition_to_position_open(
        ...
        if not self.validator.validate_transition(plan.status, TradePlanStatus.POSITION_OPEN):
    ...
    async def transition_to_completed(
        ...
        if not self.validator.validate_transition(plan.status, TradePlanStatus.COMPLETED):
            raise StateTransitionError(
    ...
    async def transition_to_cancelled(
        ...
        if not self.validator.validate_transition(plan.status, TradePlanStatus.CANCELLED):
            raise StateTransitionError(
