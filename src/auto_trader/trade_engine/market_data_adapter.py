...
class MarketDataExecutionAdapter:
    ...
    async def on_market_data_update(self, bar: BarData) -> None:
        ...
        try:
            ...
            if not validation_result.is_valid:
                error_msg = (
                    f"Invalid market data detected: {validation_result.error_message}"
                )
                logger.error(
                    error_msg,
                    symbol=bar.symbol,
            ...
            if timeframe == Timeframe.ONE_MIN:
                ...
                await self.historical_data_manager.store_one_minute_for_all_timeframes(
                    bar, monitored
                )
            ...
            logger.error(
                f"Error processing market data update: {e}", bar=bar.model_dump()
            )
    ...
    async def _on_bar_close(self, event: BarCloseEvent) -> None:
        ...
        try:
            ...
            functions = self.function_registry.get_functions_by_timeframe(
                timeframe.value
            )
            ...
            historical_bars = await self.historical_data_manager.get_historical_bars(
                symbol, timeframe
            )

            if not await self.historical_data_manager.has_sufficient_data(
                symbol, timeframe
            ):
                logger.warning(
            ...
            logger.error(
                f"Error processing bar close event: {e}", event=event.model_dump()
            )
    ...
    def _convert_bar_size_to_timeframe(
        self, bar_size: BarSizeType
    ) -> Optional[Timeframe]:
