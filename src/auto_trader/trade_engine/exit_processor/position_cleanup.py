...
class PositionCleanupManager:
    ...
    def __init__(
        self, position_manager: PositionStateManager, risk_manager: RiskManager
    ):
    ...
    async def handle_exit_fill(
        ...
        try:
            ...
            )  # Negative for position reduction
    ...
    async def handle_order_fill_event(
        ...
        try:
            ...
            if not trade_plan:
                logger.warning(
                    f"No trade plan found for position {position.position_id}"
                )
                return

            # Process the fill
            await self.handle_exit_fill(
                position, order_result, trade_plan, processing_stats
            )

        except Exception as e:
            logger.error(
                f"Error handling order fill event {order_result.order_id}: {e}"
            )
