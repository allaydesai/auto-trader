...
class SignalValidator:
    ...
    def validate_signal_quality(
        ...
        if not signal.reasoning or len(signal.reasoning.strip()) == 0:
            logger.debug(
                "Signal rejected: missing reasoning", plan_id=trade_plan.plan_id
            )
            return False
        ...
        if not signal.metadata:
            logger.debug(
                "Signal rejected: missing metadata", plan_id=trade_plan.plan_id
            )
            return False
    ...
    def check_duplicate_signal(
        ...
        signal_key = (
            f"{trade_plan.plan_id}_{signal.action.value}_{signal.confidence:.2f}"
        )

        current_time = datetime.now(UTC)
        cutoff_time = current_time - timedelta(
            seconds=self.config.duplicate_signal_window_seconds
        )
    ...
    def get_validation_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "oldest_signal": min(self.recent_signals.values()).isoformat()
            if self.recent_signals
            else None,
            "newest_signal": max(self.recent_signals.values()).isoformat()
            if self.recent_signals
            else None,
        }
